...
async function readFreshOracle(provider, address, label, maxAgeSeconds, nowSeconds) {
  ...
  return {
    address: normalized,
    priceWad: normalizeOraclePriceWad(answer, Number(decimals)),
    updatedAt,
    ageSeconds,
...
export function findExactRobinhoodManifestCandidate({ chainId, contractAddress }) {
  ...
  return findManifestAsset({
    chainId: String(ROBINHOOD_MAINNET_CHAIN_ID),
...
export async function certifyRobinhoodStockRuntime({ row, provider, factoryAddress, now = Date.now(), probeNativeWei = certificationProbeNativeWei() }) {
  ...
  if (String(manifestAsset.symbol).toUpperCase() !== String(row?.symbol || manifestAsset.symbol).toUpperCase()) {
    throw new Error("provider symbol does not match exact approved manifest identity");
  }
  ...
  const [adapterRaw, implementationRaw, factoryLockerRaw] = await Promise.all([
  ...
  const [
    ...
    adapter.swapRouter(),
    adapter.WETH(),
    adapter.permanentPositionLocker(),
    adapter.nativeUsdOracle(),
    adapter.feeTier(),
    adapter.maxOracleAgeSeconds(),
    adapter.campaignFactory(),
    adapter.campaignFactoryLocked(),
    adapter.stockRoutes(tokenAddress),
  ]);
  ...
  const [canonicalAcquisitionPool, acquisitionSpacing, memeSpacing] = await Promise.all([
    v3Factory.getPool(weth, tokenAddress, acquisitionFeeTier),
    v3Factory.feeAmountTickSpacing(acquisitionFeeTier),
    v3Factory.feeAmountTickSpacing(Number(memeFeeTierRaw)),
  ]);
  ...
  const [tokenDecimalsRaw, poolStockBalance] = await Promise.all([
    token.decimals(),
    token.balanceOf(acquisitionPool),
  ]);
  ...
  const [stockOracle, nativeOracle] = await Promise.all([
    readFreshOracle(provider, oracleFeed, "Stock/USD oracle", maxOracleAgeSeconds, nowSeconds),
    readFreshOracle(provider, nativeOracleAddress, "Native/USD oracle", maxOracleAgeSeconds, nowSeconds),
  ]);

  const poolStockUsdWad = (BigInt(poolStockBalance) * stockOracle.priceWad) / pow10(tokenDecimals);
  if (poolStockUsdWad < minimumRouteLiquidityUsdWad) {
    throw new Error(`acquisition pool liquidity below route minimum (${poolStockUsdWad} < ${minimumRouteLiquidityUsdWad})`);
  }
  ...
  const [quotedOutRaw, probeOutRaw] = await Promise.all([
    router.quoteExactInputSingle(weth, tokenAddress, acquisitionFeeTier, probeNativeWei),
    router.quoteExactInputSingle(weth, tokenAddress, acquisitionFeeTier, probeIn),
  ]);
  ...
  const [lockerAuthorized, lockerFactory, lockerManager, lockerWeth, lockerFeeTier] = await Promise.all([
    locker.authorizedIntegrationSource(adapterAddress),
    locker.v3Factory(),
    locker.positionManager(),
    locker.wrappedNative(),
    locker.configuredFeeTier(),
  ]);
