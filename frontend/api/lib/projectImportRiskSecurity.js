...
function add(list, condition, code, label) { if (condition) list.push({ code, label }); }
...
function bnbAssessment(raw) {
  ...
  return { critical, review, details: { buyTax, sellTax, holderCount: raw?.holder_count ?? null } };
}
...
function solanaAssessment(raw) {
  ...
  add(critical, flag(raw?.transfer_hook?.malicious_address), "malicious_transfer_hook", "Transfer hook is flagged malicious");
  ...
  add(review, nestedFlag(raw?.closable), "closable", "Token program/accounts can be closed by authority");
  ...
  add(review, Boolean(raw?.transfer_hook?.address), "transfer_hook", "External transfer hook is present");
  ...
  return { critical, review, details: { transferFeeBps: currentFeeBps, trustedToken: raw?.trusted_token ?? null } };
}
...
export async function scanProjectImportSecurity({ chainId, tokenAddress, fetchImpl = fetch }) {
  ...
  const url = id === 56
    ? `${GOPLUS_BASE}/token_security/56?contract_addresses=${encodeURIComponent(token)}`
    : id === 101
