...
function jpegInfo(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (segmentLength < 2 || offset + segmentLength > buffer.length) throw new Error("JPEG contains an invalid segment length");
    if (JPEG_SOF_MARKERS.has(marker)) {
      if (segmentLength < 7 || offset + 7 > buffer.length) throw new Error("JPEG SOF header is truncated");
      return { mime: "image/jpeg", ext: "jpg", height: buffer.readUInt16BE(offset + 3), width: buffer.readUInt16BE(offset + 5) };
    }
...
function webpInfo(buffer) {
  ...
  if (chunk === "VP8X") return { mime: "image/webp", ext: "webp", width: 1 + readUInt24LE(buffer, 24), height: 1 + readUInt24LE(buffer, 27) };
  if (chunk === "VP8L") {
    ...
    const b3 = buffer[23];
    const b4 = buffer[24];
    return { mime: "image/webp", ext: "webp", width: 1 + (((b2 & 0x3f) << 8) | b1), height: 1 + (((b4 & 0x0f) << 10) | (b3 << 2) | ((b2 & 0xc0) >> 6)) };
  }
  if (chunk === "VP8 ") {
    if (buffer[23] !== 0x9d || buffer[24] !== 0x01 || buffer[25] !== 0x2a) throw new Error("WEBP lossy frame signature is invalid");
    return { mime: "image/webp", ext: "webp", width: buffer.readUInt16LE(26) & 0x3fff, height: buffer.readUInt16LE(28) & 0x3fff };
  }
...
export function inspectImageFile(buffer, options = {}) {
  ...
  if (declared && declared !== detected.mime) throw new Error(`Image MIME mismatch: declared ${declared}, detected ${detected.mime}`);
  if (!(detected.width > 0) || !(detected.height > 0)) throw new Error("Image dimensions are invalid");
  if (detected.width > maxDimension || detected.height > maxDimension) throw new Error(`Image dimensions exceed ${maxDimension}x${maxDimension}`);
  if (detected.width * detected.height > maxPixels) throw new Error("Image pixel count is too large");
  return detected;
}

export const PROJECT_IMPORT_IMAGE_LIMITS = Object.freeze({
  maxBytes: DEFAULT_MAX_BYTES,
  maxDimension: DEFAULT_MAX_DIMENSION,
  maxPixels: DEFAULT_MAX_PIXELS,
});

export const ARENA_IMPORT_IMAGE_LIMITS = PROJECT_IMPORT_IMAGE_LIMITS;
