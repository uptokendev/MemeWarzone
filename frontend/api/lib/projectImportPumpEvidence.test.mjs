...
import {pumpGlobalAddress,decodePumpGlobal,PUMP_PROGRAM,PUMP_FEES_PROGRAM,PUMP_AMM_PROGRAM,WRAPPED_SOL,pumpCurveAddress,pumpSharingAddress,pumpPoolAddress,decodePumpCurve,decodePumpSharing,decodeCanonicalPumpPool,inspectCustodyTokenAccount,readPumpImportEvidence} from './projectImportPumpEvidence.js';
...
function globalAccount(flags=0){const b=Buffer.alloc(57);Buffer.from([149,8,156,202,160,252,176,217]).copy(b);b[56]=flags;return info(b,PUMP_AMM_PROGRAM);}
...
test('completed curve alone does not prove graduation market readiness',async()=>{const e=await readPumpImportEvidence({connection:connection(new Map()),mint,curveAccount:curve(claimant,true),claimant:claimant.toBase58()});assert.equal(e.market.phase,'postgrad_unverified');});
test('canonical pool validates authority, mint, index, bump and funded custody',async()=>{
 const p=pool(),map=new Map([[pumpGlobalAddress().toBase58(),globalAccount()],[p.address.toBase58(),p.account],[p.baseAta.toBase58(),token(p.address)],[p.quoteAta.toBase58(),token(p.address,500n,WRAPPED_SOL,TOKEN_PROGRAM_ID)]]);
 assert.ok(decodeCanonicalPumpPool(p.account,mint));
 for(const off of [8,9,11,43,75,243]){const b=Buffer.from(p.account.data);b[off]^=1;assert.equal(decodeCanonicalPumpPool(info(b,PUMP_AMM_PROGRAM),mint),null,`pool offset ${off}`);}
...
test('wrong program, malformed flags, pool lookup outage and non-native quote remain fail closed',async()=>{
 ...
 const q=curve(claimant,true);Keypair.generate().publicKey.toBuffer().copy(q.data,83);const other=await readPumpImportEvidence({connection:connection(new Map()),mint,curveAccount:q});assert.equal(other.market.phase,'postgrad_unverified');
});

test('signed virtual reserve extension is supported, never counted as funded liquidity',async()=>{
 const p=pool();p.account.data.writeBigUInt64LE(5000n,245);
 assert.equal(decodeCanonicalPumpPool(p.account,mint).virtualQuoteReserves,'5000');
 const map=new Map([[pumpGlobalAddress().toBase58(),globalAccount()],[p.address.toBase58(),p.account],[p.baseAta.toBase58(),token(p.address)],[p.quoteAta.toBase58(),token(p.address,0n,WRAPPED_SOL,TOKEN_PROGRAM_ID)]]);
 const e=await readPumpImportEvidence({connection:connection(map),mint,curveAccount:curve(claimant,true)});
 assert.equal(e.market.phase,'postgrad');assert.equal(e.market.quoteReserve,'0');assert.equal(e.market.effectiveQuoteReserves,'5000');assert.equal(e.market.liquidityAvailable,false);assert.equal(e.market.executionTested,false);
 p.account.data.fill(255,245,261);assert.equal(decodeCanonicalPumpPool(p.account,mint).virtualQuoteReserves,'-1');
 assert.equal(decodeCanonicalPumpPool(info(p.account.data.subarray(0,250),PUMP_AMM_PROGRAM),mint),null);
});
test('global buy and sell restrictions are not a market or safety clearance',()=>{
 assert.deepEqual(decodePumpGlobal(globalAccount(24)),{buyEnabled:false,sellEnabled:false,disableFlags:24});assert.equal(decodePumpGlobal({...globalAccount(),owner:TOKEN_PROGRAM_ID}),null);
});
