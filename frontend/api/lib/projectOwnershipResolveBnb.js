...
async function optionalRead(provider, contractAddress, method) {
  try {
    return { readable: true, value: await callRead(provider, contractAddress, method) };
  } catch {
...
async function resolveCurrentOwner(provider, contractAddress, signedConnectedWallet) {
  ...
  let currentOwner;
  try {
    currentOwner = normalizeAddress(rawOwner, "current owner");
  } catch {
    return unavailableOwnership(method);
  }
  ...
  return {
    ...
    message: verified
      ? "signed connected wallet matches current owner"
      : "signed connected wallet does not match current owner",
  };
}

export async function resolveProjectOwnershipBnb({
  provider,
  chainId,
  contractAddress,
  signedConnectedWallet,
}) {
  if (Number(chainId) !== BNB_PROJECT_OWNERSHIP_CHAIN_ID) {
    throw new TypeError("BNB ownership resolver only supports chain 56.");
  }
  if (!provider || typeof provider.getCode !== "function" || typeof provider.call !== "function") {
    throw new TypeError("A read-only EVM provider is required.");
  }
  ...
  try {
    ...
    return {
      ok: false,
      chainId: BNB_PROJECT_OWNERSHIP_CHAIN_ID,
      contractAddress: tokenAddress,
      errorCode: "BYTECODE_CHECK_FAILED",
      error: String(error?.message || error || "Bytecode check failed."),
    };
  }

  if (!hasDeployedBytecode(bytecode)) {
    return {
      ok: false,
      chainId: BNB_PROJECT_OWNERSHIP_CHAIN_ID,
      contractAddress: tokenAddress,
      errorCode: "NO_DEPLOYED_BYTECODE",
      error: "No deployed bytecode exists at the supplied contract address.",
    };
  }
  ...
  if (!symbolRead.readable || !decimalsRead.readable) {
    return {
      ok: false,
      chainId: BNB_PROJECT_OWNERSHIP_CHAIN_ID,
      contractAddress: tokenAddress,
      errorCode: "ERC20_READ_FAILED",
      error: "Required ERC20 symbol() or decimals() read is unavailable.",
    };
  }
  ...
  if (!Number.isInteger(decimals) || decimals < 0 || decimals > 255) {
    return {
      ...
      chainId: BNB_PROJECT_OWNERSHIP_CHAIN_ID,
      contractAddress: tokenAddress,
      errorCode: "ERC20_DECIMALS_INVALID",
      error: "ERC20 decimals() returned an invalid value.",
    };
  }
