...
function rpcUrl(chainId) {
  ...
  const perChain = chain === 4663 || chain === 46630
    ? String(process.env[`ROBINHOOD_RPC_HTTP_${chain}`] || (chain === 4663 ? process.env.ROBINHOOD_MAINNET_RPC_URL : process.env.ROBINHOOD_TESTNET_RPC_URL) || "").trim()
...
function providerForChain(chainId) {
  const chain = Number(chainId);
  if (!EVM_LEAGUE_CHAINS.has(chain)) throw new EvmLeagueClaimVerificationError("LEAGUE_CHAIN_UNSUPPORTED", "Unsupported EVM League claim chain.", 400);
  const network = Network.from(chain);
  return new JsonRpcProvider(rpcUrl(chain), network, { staticNetwork: network, batchMaxCount: 1, batchStallTime: 0 });
}
...
export function buildExpectedEvmLeagueClaim({ chainId, period, epochStart, category, rank, recipient, amountRaw }) {
  ...
  if (Number.isNaN(epoch.getTime())) throw new EvmLeagueClaimVerificationError("LEAGUE_EPOCH_INVALID", "Invalid League epochStart.", 400);
...
export function evmLeagueClaimEventTopics(expected) {
  return [EVM_LEAGUE_INTERFACE.getEvent("Claimed").topicHash, expected.epochIdHex, zeroPadValue(expected.recipient, 32), expected.leaf];
}
...
export async function verifyEvmLeagueClaimTransaction({ chainId, period, epochStart, category, rank, recipient, amountRaw, txHash, minConfirmations = 1 }) {
  ...
  if (!sameAddress(tx.to, expected.vaultAddress)) throw new EvmLeagueClaimVerificationError("LEAGUE_VAULT_MISMATCH", "League claim targeted the wrong TreasuryVaultV2 contract.");
  let parsed;
  try { parsed = EVM_LEAGUE_INTERFACE.parseTransaction({ data: tx.data, value: tx.value }); } catch { parsed = null; }
  if (!parsed || parsed.name !== "claim") throw new EvmLeagueClaimVerificationError("LEAGUE_CALL_MISMATCH", "Transaction is not a TreasuryVaultV2 League claim.");
  if (BigInt(parsed.args?.[0] ?? 0n) !== expected.epochId) throw new EvmLeagueClaimVerificationError("LEAGUE_EPOCH_MISMATCH", "League claim epoch does not match the entitlement.");
  if (!sameBytes32(parsed.args?.[1], expected.categoryHash)) throw new EvmLeagueClaimVerificationError("LEAGUE_CATEGORY_MISMATCH", "League claim category does not match the entitlement.");
  if (Number(parsed.args?.[2]) !== expected.rank) throw new EvmLeagueClaimVerificationError("LEAGUE_RANK_MISMATCH", "League claim rank does not match the entitlement.");
  if (!sameAddress(parsed.args?.[3], expected.recipient)) throw new EvmLeagueClaimVerificationError("LEAGUE_RECIPIENT_MISMATCH", "League claim recipient does not match the winner.");
  if (BigInt(parsed.args?.[4] ?? 0n) !== BigInt(expected.amountRaw)) throw new EvmLeagueClaimVerificationError("LEAGUE_AMOUNT_MISMATCH", "League claim amount does not match the entitlement.");
  ...
  for (const log of receipt.logs || []) {
    ...
    try {
      ...
      if (event?.name === "Claimed" && BigInt(event.args?.[0] ?? 0n) === expected.epochId && sameAddress(event.args?.[1], expected.recipient) && BigInt(event.args?.[2] ?? 0n) === BigInt(expected.amountRaw) && sameBytes32(event.args?.[3], expected.leaf)) { matchedEvent = true; break; }
    } catch {}
  }
  if (!matchedEvent) throw new EvmLeagueClaimVerificationError("LEAGUE_EVENT_MISSING", "Confirmed transaction does not contain the exact expected League Claimed event.");
  ...
  if (confirmations < requiredConfirmations) throw new EvmLeagueClaimVerificationError("LEAGUE_CONFIRMATIONS_PENDING", `League claim has ${confirmations}/${requiredConfirmations} required confirmations.`, 409);
