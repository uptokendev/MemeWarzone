...
const PROFILE_A = profile({ tokenId: TOKEN_A, tokenAddress: TOKEN_A, ownerWallet: "owner-a", marketCapUsd: 12_000_000, holderCount: 820, liquidityUsd: 720_000, volumeUsd: 190_000, launchedAt: "2026-08-01T00:00:00.000Z" });
const PROFILE_B = profile({ tokenId: TOKEN_B, tokenAddress: TOKEN_B, ownerWallet: "owner-b", marketCapUsd: 12_600_000, holderCount: 850, liquidityUsd: 700_000, volumeUsd: 205_000, launchedAt: "2026-08-03T00:00:00.000Z" });
const PROFILE_C = profile({ tokenId: TOKEN_C, tokenAddress: TOKEN_C, ownerWallet: "owner-c", marketCapUsd: 62_000_000, holderCount: 5_400, liquidityUsd: 4_200_000, volumeUsd: 930_000, launchedAt: "2026-07-20T00:00:00.000Z" });
const PROFILE_D = profile({ tokenId: TOKEN_D, tokenAddress: TOKEN_D, ownerWallet: "owner-d", marketCapUsd: 60_000_000, holderCount: 5_200, liquidityUsd: 4_050_000, volumeUsd: 910_000, launchedAt: "2026-07-19T00:00:00.000Z" });

const PROFILE_BY_TOKEN = new Map([[TOKEN_A, PROFILE_A], [TOKEN_B, PROFILE_B], [TOKEN_C, PROFILE_C], [TOKEN_D, PROFILE_D]]);

function pairingsAsKeys(pairings) {
  return pairings.map((pair) => unorderedPairKey(pair.left.tokenAddress, pair.right.tokenAddress)).sort();
}
...
test("runtime acceptance: ranked Solana battles refresh the deposit window, go live, and settle canonically", () => {
  ...
  const lifecycle = solanaMatchedLifecyclePatch(waiting, { state: "matched", started_at: null, ends_at: null }, { nowMs: NOW_MS, depositEndsAt: DEPOSIT_DEADLINE });
  assert.equal(lifecycle.action, "refresh-deposit");
  assert.deepEqual(lifecycle.patch, { state: "matched", started_at: null, ends_at: DEPOSIT_DEADLINE });
  ...
  const battle = { state: "live", ends_at: LIVE_ENDS_AT, participants: [{ tokenId: TOKEN_A, tokenAddress: TOKEN_A }, { tokenId: TOKEN_B, tokenAddress: TOKEN_B }] };
  assert.equal(canSettleBattle(battle, Date.parse(SETTLED_AT)), true);

  const decision = decideBattleSettlement({ leftToken: TOKEN_A, rightToken: TOKEN_B, leftStartMcap: PROFILE_A.marketCapUsd, rightStartMcap: PROFILE_B.marketCapUsd, leftEndMcap: 14_000_000, rightEndMcap: 13_000_000 });
...
test("runtime acceptance: paid tournament rosters seed the closest markets together", () => {
  const blocked = tournamentStartRoster([{ tokenAddress: TOKEN_A, buyInIntent: true, buyInPaid: true }, { tokenAddress: TOKEN_B, buyInIntent: true, buyInPaid: false }], { buyInNative: 0.25 });
  assert.equal(blocked.ok, false);
  assert.equal(blocked.reason, "unpaid-roster");

  const paidRoster = tournamentStartRoster([{ tokenAddress: TOKEN_A, buyInIntent: true, buyInPaid: true }, { tokenAddress: TOKEN_B, buyInIntent: true, buyInPaid: true }, { tokenAddress: TOKEN_C, buyInIntent: true, buyInPaid: true }, { tokenAddress: TOKEN_D, buyInIntent: true, buyInPaid: true }], { buyInNative: 0.25 });
  assert.equal(paidRoster.ok, true);
  assert.equal(paidRoster.reason, "paid");

  const seeded = optimizeMatchPairings(paidRoster.roster, { nowMs: NOW_MS, getProfile: (entry) => PROFILE_BY_TOKEN.get(entry.tokenAddress) });
  assert.equal(seeded.pairings.length, 2);
  assert.equal(seeded.bye, null);
  assert.deepEqual(pairingsAsKeys(seeded.pairings), [unorderedPairKey(TOKEN_A, TOKEN_B), unorderedPairKey(TOKEN_C, TOKEN_D)].sort());
  assert.ok(seeded.totalMatchQuality > 140);
  for (const pairing of seeded.pairings) { assert.equal(pairing.ranked, true); assert.notEqual(pairing.classification, "open_war"); }
});

test("runtime acceptance: tournament progression advances with the money winner even when MWL draws", () => {
  const decision = decideBattleSettlement({ leftToken: TOKEN_A, rightToken: TOKEN_B, leftStartMcap: 100, rightStartMcap: 200, leftEndMcap: 110, rightEndMcap: 220 });
  assert.equal(decision.ok, true);
  ...
  const settled = battleSettlementPatch(decision, { nowIso: SETTLED_AT, participants: [{ tokenId: TOKEN_A, tokenAddress: TOKEN_A }, { tokenId: TOKEN_B, tokenAddress: TOKEN_B }] });
  const planned = planTournamentBracketReconcile({
    tournament: { id: "tourney-final", status: "live", chain_id: SOLANA_CHAIN_ID, winner_token: null, bracket: { rounds: [{ round: 1, matches: [{ id: "m1", tokenA: TOKEN_A, tokenB: TOKEN_B, battleId: "arena-final-1", winner: null, bye: false }] }] } },
    battle: { id: "arena-final-1", tournament_id: "tourney-final", source: "tournament", state: "finished", chain_id: SOLANA_CHAIN_ID, challenger_token: TOKEN_A, defender_token: TOKEN_B, winner_token: settled.patch.winner_token, money_winner_token: settled.patch.money_winner_token, mwl_winner_token: settled.patch.mwl_winner_token, mwl_draw: settled.patch.mwl_draw },
  });
