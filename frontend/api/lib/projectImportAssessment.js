...
export const IMPORT_REVIEW_POLICY = 'import_review_v1_20260909';
...
export function assessProjectImport({resolved,security,claimantWallet,proof=null,checkedAt=new Date().toISOString()}) {
  ...
  const postgrad=market.verified===true&&market.phase==='postgrad'&&market.liquidityAvailable===true;
  ...
  const checks=[
    ...
    {key:'market',status:bonding?'blocked':postgrad?'pass':'unknown',title:'Market stage',finding:bonding?'Still bonding on Pump.fun.':postgrad?'Canonical PumpSwap pool and funded custody accounts verified.':market.phase==='migration_pending'?'Bonding completed; a supported funded post-grad pool is not verified.':'Launch stage or market is not supported by the current automatic verifier.',meaning:'Bonding tokens cannot be newly imported for the post-grad system. A detected pool is not a trading approval.',nextAction:bonding?'Return after graduation.':postgrad?'Continue ownership and safety review.':'Escalate for a supported market verifier; do not approve on a chart alone.'},
  ...
  const automaticImportAllowed=match&&postgrad&&safe&&!technicalFailure;
  const manualRequestAllowed=!bonding&&!mismatch&&!automaticImportAllowed;
  const decision=bonding?'not_eligible':mismatch?'wrong_wallet':technicalFailure||!postgrad?'technical_review':automaticImportAllowed?'automatic':'manual_review';
  return {
    schemaVersion:1,policyVersion:IMPORT_REVIEW_POLICY,checkedAt,...identity,claimantWallet,
    proof,observedSlot:resolved.observedSlot??null,
    ...
    canVerifyOwner:!bonding&&!mismatch&&postgrad&&!technicalFailure&&['pass','review'].includes(security?.status),
...
export function assertReviewApproval(snapshot,{project,evidenceId,expectedEvidenceId,now=Date.now(),reviewProof}) {
  ...
  if(snapshot.chainId!==Number(project.chain_id)||snapshot.tokenAddress!==project.token_address||snapshot.claimantWallet!==project.manual_claim_wallet)throw error('Evidence belongs to another project or claimant.','PROJECT_IMPORT_EVIDENCE_MISMATCH');
  ...
  if(!snapshot.canVerifyOwner||snapshot.market?.phase!=='postgrad'||snapshot.market?.verified!==true||snapshot.market?.liquidityAvailable!==true||snapshot.authority?.status==='mismatch'||!['pass','review'].includes(snapshot.security?.status))throw error('The checks require technical review. Ownership approval cannot override them.','PROJECT_IMPORT_TECHNICAL_REVIEW');
