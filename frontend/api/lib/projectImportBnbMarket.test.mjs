...
test('ordinary DEX market uncertainty does not block matching Project ownership',async()=>{
 ...
 const a=assessment(result);assert.equal(a.canVerifyOwner,true);assert.equal(a.automaticImportAllowed,true);assert.equal(a.decision,'automatic');assert.equal(a.permissions.trading,'locked');assert.equal(a.permissions.battle,'locked');assert.throws(()=>approve(a),{code:'PROJECT_IMPORT_MARKET_PROOF_REQUIRED'});
...
for(const options of [{wrongFactory:true},{wrongMint:true},{empty:true},{unbacked:true},{noCode:[pair]},{venue:'v3',locked:true},{venue:'v3',inactive:true},{venue:'v3',wrongFee:true},{venue:'v3',uninitialized:true}])test(`invalid or unfunded market stays locked while matching Project ownership can register: ${JSON.stringify(options)}`,async()=>{
 const {result}=await read({four:true,graduated:true,...options});const a=assessment(result);assert.equal(result.market.verified,false);assert.equal(a.canVerifyOwner,true);assert.equal(a.automaticImportAllowed,true);assert.equal(a.permissions.trading,'locked');assert.equal(a.permissions.battle,'locked');assert.throws(()=>approve(a),{code:'PROJECT_IMPORT_TECHNICAL_REVIEW'});
});
test('market verifier failures remain separate from matching Project ownership',async()=>{
 for(const o of [{registryError:true},{four:true,version:9},{four:true,manager:pair},{four:true,graduated:true,quote:wallet},{four:true,graduated:true,noPool:true}]){
  const {result}=await read(o);const a=assessment(result);assert.notEqual(result.market.phase,'bonding');assert.equal(result.market.verified,false);assert.equal(a.canVerifyOwner,true);assert.equal(a.automaticImportAllowed,true);assert.equal(a.permissions.trading,'locked');assert.equal(a.permissions.battle,'locked');assert.throws(()=>approve(a),{code:'PROJECT_IMPORT_TECHNICAL_REVIEW'});
 }
});
