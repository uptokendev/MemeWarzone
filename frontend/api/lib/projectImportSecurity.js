...
export function assertVerifiedProjectOwner(project, { wallet, chainId, token } = {}) {
  ...
  const requestedIdentity = canonicalProjectImportIdentity(chainId ?? projectIdentity.chainId, token ?? projectIdentity.token);
...
export function sanitizeProjectImportMetadataPatch(patch) {
  ...
  for (const [key, value] of Object.entries(patch)) {
    if (FORBIDDEN_METADATA_KEYS.has(key) || !METADATA_FIELDS.has(key)) {
      throw Object.assign(new Error(`Metadata field is not editable: ${key}`), { code: "IMPORT_METADATA_FIELD_FORBIDDEN", field: key });
    }
...
export function assertNoImportSideEffectMutation(before, after) {
  const protectedFields = [
    "chain_id", "chainId", "token_address", "tokenAddress", "mint",
    "arena_status", "arenaStatus", "arena_eligible", "arenaEligible",
    "campaign_id", "campaignId", "payout", "payout_amount", "rewards",
    "creator_economics", "creatorEconomics", "graduation_eligible", "graduationEligible",
  ];
  for (const field of protectedFields) {
    if (Object.prototype.hasOwnProperty.call(before || {}, field) || Object.prototype.hasOwnProperty.call(after || {}, field)) {
      if (canonicalJson(before?.[field]) !== canonicalJson(after?.[field])) {
        throw Object.assign(new Error(`Project import mutation changed protected field: ${field}`), { code: "IMPORT_SIDE_EFFECT_FORBIDDEN", field });
      }
    }
  }
  return true;
