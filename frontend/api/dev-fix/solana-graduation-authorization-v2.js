...
function parseQuoteCatalog() {
  const native = {
    ...
    mint: NATIVE_MINT,
    policyVersion: 1,
    profile: QUOTE_PROFILE.NATIVE,
    providerClass: 0,
    decimals: 9,
    acquisitionProgram: SYSTEM_PROGRAM_ID,
    recoveryAccount: SYSTEM_PROGRAM_ID,
    activeInBasicRelease: true,
    maxSlippageBps: 0,
    maxImpactBps: 0,
    maxDeviationBps: 0,
  };
  const raw = String(process.env.SOLANA_GRADUATION_QUOTE_CATALOG_JSON || "").trim();
  let configured = [];
  if (raw) {
    try {
      configured = JSON.parse(raw);
    } catch (error) {
      throw new SolanaGraduationAuthorizationError("SOLANA_GRADUATION_QUOTE_CATALOG_JSON is invalid JSON.", {
        code: "SOLANA_GRADUATION_CONFIGURATION_INCOMPLETE",
        httpStatus: 503,
        cause: error,
      });
    ...
    if (!Array.isArray(configured)) {
      throw new SolanaGraduationAuthorizationError("SOLANA_GRADUATION_QUOTE_CATALOG_JSON must be an array.", {
        code: "SOLANA_GRADUATION_CONFIGURATION_INCOMPLETE",
        httpStatus: 503,
      });
    }
  }
  const normalized = [native, ...configured].map((entry) => ({
    id: String(entry.id || "").trim(),
    mint: publicKeyString(entry.mint, `quote catalog ${entry.id || "entry"} mint`),
    policyVersion: parsePositiveInteger(entry.policyVersion, 1, 65_535),
    profile: Number(entry.profile),
    providerClass: Number(entry.providerClass),
    decimals: Number(entry.decimals),
    acquisitionProgram: publicKeyString(entry.acquisitionProgram || SYSTEM_PROGRAM_ID, "acquisitionProgram"),
    recoveryAccount: publicKeyString(entry.recoveryAccount || SYSTEM_PROGRAM_ID, "recoveryAccount"),
    activeInBasicRelease: entry.activeInBasicRelease !== false,
    maxSlippageBps: Number(entry.maxSlippageBps ?? DEFAULT_SLIPPAGE_BPS),
    maxImpactBps: Number(entry.maxImpactBps ?? 100),
    maxDeviationBps: Number(entry.maxDeviationBps ?? 100),
    quoteUsdMicros: entry.quoteUsdMicros == null ? null : BigInt(entry.quoteUsdMicros),
    binanceSymbol: String(entry.binanceSymbol || "").trim(),
    coinGeckoId: String(entry.coinGeckoId || "").trim(),
    jupiterApiBase: String(entry.jupiterApiBase || process.env.SOLANA_GRADUATION_JUPITER_API_BASE || "https://lite-api.jup.ag/swap/v1").replace(/\/$/, ""),
  }));

  const seenIds = new Set();
  const seenMints = new Set();
  for (const entry of normalized) {
    if (!entry.id || seenIds.has(entry.id) || seenMints.has(entry.mint)) {
      throw new SolanaGraduationAuthorizationError("Quote catalog contains a missing/duplicate id or mint.", {
        code: "SOLANA_GRADUATION_CONFIGURATION_INCOMPLETE",
        httpStatus: 503,
      });
    }
    ...
    if (!Number.isInteger(entry.profile) || entry.profile < 0 || entry.profile > 4) throw new Error(`Invalid quote profile for ${entry.id}`);
    if (!Number.isInteger(entry.providerClass) || entry.providerClass < 0 || entry.providerClass > 255) throw new Error(`Invalid provider class for ${entry.id}`);
    if (!Number.isInteger(entry.decimals) || entry.decimals < 0 || entry.decimals > 18) throw new Error(`Invalid decimals for ${entry.id}`);
    if (entry.maxSlippageBps < 0 || entry.maxSlippageBps > ABSOLUTE_MAX_SLIPPAGE_BPS) throw new Error(`Invalid maxSlippageBps for ${entry.id}`);
    if (entry.maxImpactBps < 0 || entry.maxImpactBps > ABSOLUTE_MAX_IMPACT_BPS) throw new Error(`Invalid maxImpactBps for ${entry.id}`);
    if (entry.maxDeviationBps < 0 || entry.maxDeviationBps > ABSOLUTE_MAX_DEVIATION_BPS) throw new Error(`Invalid maxDeviationBps for ${entry.id}`);
  }

  if (isTruthy(process.env.SOLANA_GRADUATION_BASIC_RELEASE_ONLY, true)) {
    const nonNativeActive = normalized.filter((entry) => entry.activeInBasicRelease && entry.profile !== QUOTE_PROFILE.NATIVE);
    if (nonNativeActive.length > 1 || nonNativeActive.some((entry) => entry.profile !== QUOTE_PROFILE.STABLECOIN)) {
      throw new SolanaGraduationAuthorizationError("BASIC release permits SOL plus exactly zero or one active STABLECOIN quote.", {
        code: "SOLANA_GRADUATION_CONFIGURATION_INCOMPLETE",
        httpStatus: 503,
      });
    }
