...
function projectError(res,error){const code=error?.code||"PROJECT_IMPORT_ERROR";return json(res,errorStatus(code),{error:String(error?.message||error),code,currentVersion:error?.currentVersion||undefined,currentOwnershipStatus:error?.currentOwnershipStatus||undefined});}
...
function claimAdminItem(row){
  ...
  return{
    ...
    ownershipEvidence:row.scan_json??null,createdAt:row.created_at??null,metadataUpdatedAt:row.metadata_updated_at??null,updatedAt:row.updated_at??null,
    stateVersion:String(row.state_version||""),
  };
}
async function listProjectOwnershipClaims(){
  const r=await pool.query(`SELECT i.*, i.xmin::text AS state_version FROM public.arena_token_imports i WHERE i.ownership_status='ownership_manual_review' AND i.manual_claim_wallet IS NOT NULL AND btrim(i.manual_claim_wallet)<>'' AND i.manual_claim_requested_at IS NOT NULL ORDER BY i.manual_claim_requested_at ASC, i.created_at ASC`);
  return r.rows||[];
}
async function getProjectOwnershipClaim(id){const r=await pool.query(`SELECT i.*, i.xmin::text AS state_version FROM public.arena_token_imports i WHERE i.id=$1 LIMIT 1`,[id]);return r.rows?.[0]||null;}
...
async function reviewProjectOwnership({projectId,action,reason,expectedVersion,admin}){
  const cleanReason=String(reason||"").trim().slice(0,1000);
  if(cleanReason.length<3)throw Object.assign(new Error("Operator reason is required"),{code:"PROJECT_OWNERSHIP_REASON_REQUIRED"});
  if(!String(expectedVersion||"").trim())throw Object.assign(new Error("Expected ownership state version is required"),{code:"PROJECT_OWNERSHIP_EXPECTED_STATE_REQUIRED"});
  ...
  try{
    ...
    if(current.ownership_status!=="ownership_manual_review"||!currentClaimant||!current.manual_claim_requested_at||String(current.state_version)!==String(expectedVersion)){
      throw Object.assign(new Error("Project ownership claim changed before review"),{code:"PROJECT_OWNERSHIP_STATE_CONFLICT",currentVersion:String(current.state_version||""),currentOwnershipStatus:current.ownership_status});
    }
    let updated;
    if(action==="verify_owner"){
      const r=await client.query(`UPDATE public.arena_token_imports SET project_owner_wallet=manual_claim_wallet,ownership_status='ownership_verified',ownership_verified_at=NOW(),manual_claim_wallet=NULL,manual_claim_requested_at=NULL,manual_claim_note=NULL,updated_at=NOW() WHERE id=$1 AND xmin::text=$2 AND ownership_status='ownership_manual_review' RETURNING *, xmin::text AS state_version`,[projectId,String(expectedVersion)]);
      updated=r.rows?.[0];
...
async function handleOwnershipAdmin(req,res,path){
  ...
  if(req.method==="POST"&&actionMatch){const body=await readJson(req);const updated=await reviewProjectOwnership({projectId:actionMatch[1],action:actionMatch[2].toLowerCase()==="verify"?"verify_owner":"reject_claim",reason:body.reason,expectedVersion:body.expectedVersion,admin});return json(res,200,{item:claimAdminItem(updated)});}
  return false;
}
