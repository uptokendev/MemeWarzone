        ...
        {!state.tournament ? (
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap gap-2">
              ...
              {state.mode && !quarterlyChampionship ? <TacticalTag label={state.mode.label} tone="sponsored" /> : null}
            </div>
            <div className="space-y-1 text-[11px] uppercase tracking-[0.14em] text-white/60">
              {card?.participantCount != null ? <div>{card.participantCount} contenders</div> : null}
              {!quarterlyChampionship && state.remaining != null ? <div>{state.remaining} remaining</div> : null}
              {!quarterlyChampionship && card?.bracketStage ? <div>{String(card.bracketStage).replaceAll("_", " ")}</div> : null}
              {!quarterlyChampionship && liveBattles.length ? (
                <div data-tournament-live-battle-count={liveBattles.length}>{liveBattles.length} battles live</div>
              ) : null}
              {quarterlyChampionship ? <div>Standings continue through the quarterly epoch.</div> : null}
            </div>
            {!quarterlyChampionship ? (
              <>
