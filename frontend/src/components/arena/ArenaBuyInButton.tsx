...
async function fetchBuyInStatus(tournamentId: string, tokenAddress: string, walletAddress: string, chainId: number) {
  ...
  const response = await apiFetch(`/api/arena/tournaments/${encodeURIComponent(tournamentId)}/buy-in-status?${params.toString()}`, { cache: "no-store" });
  const json = await response.json().catch(() => ({}));
  if (!response.ok || json?.ok === false) throw new Error(String(json?.error || json?.reason || `Buy-in status unavailable (${response.status})`));
  return json as { buyInPaid?: boolean; chainPaid?: boolean; chainId: number; amountRaw: string; amountNative: string; poolId: string; treasury?: string | null; recoveryAvailable?: boolean };
}

export function ArenaBuyInButton({ tournamentId, tokenAddress, chainId, poolId, configured, live: liveFlag, opened, buyInPaid, buyInNative, nativeSymbol, onDone }: {
  tournamentId: string; tokenAddress: string; chainId?: number; poolId?: string; configured?: boolean; live?: boolean; opened?: boolean; buyInPaid?: boolean; buyInNative?: number; nativeSymbol?: string; onDone?: () => void;
}) {
  ...
  if (!live) return <p className="text-sm text-muted-foreground">{isSolana ? SOLANA_WARZONE_ESCROW_NOT_LIVE : "Tournament buy-in escrow is not live on this chain."}</p>;
  if (buyInPaid) return <p className="text-sm text-muted-foreground">On-chain registration is recorded.</p>;
  if (!opened || !poolId) return <p className="text-sm text-muted-foreground">Ops opens the tournament pool first. Opt-in is intent only until then.</p>;
  ...
  async function signedReceipt(walletAddress: string, txHash = "") {
    const auth = await signArenaWalletAction({
      ...
      extraLines: [`Tournament: ${tournamentId}`, `Token: ${tokenAddress}`, txHash ? `Tx: ${txHash}` : "Reconcile: authoritative chain state"],
      walletAddress, chainId: id, evmWallet: wallet, solanaAccount,
    });
  ...
  async function registerSolana() {
    ...
    if (String(status.poolId).toLowerCase() !== String(poolId).toLowerCase()) throw new Error("Tournament pool identity changed; reload before paying.");
    ...
    const signature = await runSolanaArenaUserAction({
      ...
      build: (web3) => buildArenaBuyInV0Instruction({ web3, poolId: arenaPoolIdFromHex(poolId!), entryAsset: tokenAddress, entrant: walletAddress }),
    });
  ...
  async function registerEvm() {
    ...
    if (!walletAddress || !wallet.provider || !wallet.signer) throw new Error("Connect the registered owner EVM wallet before paying the buy-in.");
    const status = await fetchBuyInStatus(tournamentId, tokenAddress, walletAddress, id);
    if (Number(status.chainId) !== id) throw new Error("Tournament buy-in status returned the wrong chain.");
    if (status.buyInPaid || status.chainPaid) { await signedReceipt(walletAddress); return; }
    const backendTreasury = String(status.treasury || "").trim();
    const frontendTreasury = configuredTournamentTreasury(id);
    if (!backendTreasury || !frontendTreasury || getAddress(backendTreasury) !== frontendTreasury) throw new Error("Tournament Treasury identity does not match the frontend/backend chain configuration.");
    if (String(status.poolId).toLowerCase() !== String(poolId).toLowerCase()) throw new Error("Tournament pool identity changed; reload before paying.");
    const amountRaw = BigInt(String(status.amountRaw || "0"));
    if (amountRaw <= 0n) throw new Error("Tournament buy-in amount is unavailable.");
    if (Number(wallet.chainId) !== id) await requestWalletChainSwitch(wallet.provider, id as any);
  ...
  async function register() {
    ...
    setBusy(true);
    try {
      if (isSolana) await registerSolana(); else await registerEvm();
      toast.success(Number(buyInNative) > 0 ? `Buy-in recorded (${buyInNative} ${nativeSymbol || (isSolana ? "SOL" : "native")}).` : "On-chain registration recorded.");
      onDone?.();
    } catch (error) { toast.error(String((error as Error)?.message || "Buy-in failed.")); }
    finally { setBusy(false); }
  }

  return <Button size="sm" className="font-retro" disabled={busy} onClick={() => void register()}>{busy ? "Confirming..." : Number(buyInNative) > 0 ? `Pay ${buyInNative} ${nativeSymbol || (isSolana ? "SOL" : "native")} buy-in` : "Register on-chain"}</Button>;
}
