...
export function EventSponsorshipFlow({
  ...
  if (loading) return <div role="status" aria-live="polite" className="text-xs text-white/45">Loading sponsorable events…</div>;
  if (unavailable) {
    return (
      <section data-event-sponsorship-runtime="unavailable" role="status" aria-live="polite" className="space-y-3 border border-white/10 bg-black/20 p-4">
        ...
        <Button type="button" variant="outline" size="sm" onClick={() => void loadOptions()}>Retry</Button>
      </section>
    );
  }
  if (!events.length) return <div role="status" aria-live="polite" className="text-xs text-white/45">No sponsorable MemeWarzone events are open right now.</div>;
    ...
    <section data-event-sponsorship-flow="true" aria-labelledby="event-sponsorship-title" className="space-y-4 border border-white/10 bg-black/20 p-4">
      ...
      {quoteModel.valid ? (
        <div className="space-y-3 border border-white/10 bg-black/30 p-3" data-event-sponsorship-quote="true" role="status" aria-live="polite">
          <div className="flex items-center justify-between gap-3"><span className="text-[10px] uppercase tracking-[0.14em] text-white/40">Authoritative quote</span><span className="font-retro text-lg text-white/90">{quoteModel.grossNative} {quoteModel.symbol}</span></div>
          <div className="grid gap-2 sm:grid-cols-3"><FlowMetric label="70% event prize" value={`${quoteModel.eventPrizeNative ?? "—"} ${quoteModel.symbol || ""}`.trim()} /><FlowMetric label="20% marketing" value={`${quoteModel.marketingNative ?? "—"} ${quoteModel.symbol || ""}`.trim()} /><FlowMetric label="10% protocol" value={`${quoteModel.protocolNative ?? "—"} ${quoteModel.symbol || ""}`.trim()} /></div>
          <div className="flex flex-wrap items-center gap-2"><Button type="button" size="sm" className="mwz-button font-retro" disabled={busy || !runtime.submitPayment} onClick={() => void submitPayment()}>{busy ? "Confirming…" : "Pay sponsorship"}</Button><Button type="button" variant="outline" size="sm" disabled={busy} onClick={() => void refreshPayment()}>Refresh payment state</Button></div>
          <div className="text-[9px] uppercase tracking-[0.13em] text-white/35">Backend receipt verification is authoritative. The browser never calls /confirm.</div>
        </div>
      ) : null}
      {paymentModel.quoteId ? (
        <div data-event-sponsorship-payment-state="true" className="space-y-1 border border-white/10 bg-black/25 p-3" role="status" aria-live="polite"><div className="text-[10px] uppercase tracking-[0.14em] text-white/40">{paymentModel.status.label}</div>{paymentModel.eventPrizeNative != null && paymentModel.symbol ? <div className="text-sm text-white/75">{paymentModel.eventPrizeNative} {paymentModel.symbol} added to Event Prize</div> : null}{paymentModel.txHash || paymentModel.signature ? <div className="break-all text-[10px] text-white/40">{paymentModel.txHash || paymentModel.signature}</div> : null}{paymentModel.error ? <div className="text-xs text-red-200/75">{paymentModel.error}</div> : null}</div>
      ) : null}
    </section>
  );
}
...
function FlowMetric({ label, value }: { label: string; value: string }) {
  return <div className="border border-white/10 bg-black/20 p-3"><div className="text-[9px] uppercase tracking-[0.14em] text-white/35">{label}</div><div className="mt-1 font-retro text-sm text-white/85">{value}</div></div>;
}
