  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-md" data-project-x-claim-dialog="true">
      ...
      {loading?<div className="flex items-center gap-2 py-5 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin"/>Checking ownership options…</div>:<div className="space-y-4">
        ...
        {identity?<div className="rounded-lg border border-white/10 bg-white/[0.03] p-4" data-project-x-option="true"><div className="text-xs uppercase tracking-[0.14em] text-muted-foreground">Project X attached to token</div><div className="mt-2 flex items-center gap-2 font-bold text-foreground"><ShieldCheck className="h-4 w-4 text-accent"/>@{identity.username}</div><p className="mt-2 text-xs text-muted-foreground">Sign once to link this project to your connected wallet, then verify the project X account.</p><Button type="button" className="mt-3 w-full" onClick={()=>void verifyX()} disabled={starting}>{starting?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:null}VERIFY WITH X</Button></div>:<div className="rounded-lg border border-orange-400/20 bg-orange-500/[0.04] p-4 text-xs text-orange-100">No X account is attached to this token metadata. You can still verify the creator wallet or request manual review.</div>}
