...
export function ProjectImportPanel({
  ...
  const manualReviewReason=ownershipUnavailable||securityBlocked||securityReview||marketReview;
  const canRequestManual=lookupComplete&&Boolean(evidence)&&!wrongAuthorityWallet&&!conflictingVerified&&!suspended&&!approvedOwner&&!manualReviewPending&&manualReviewReason&&!stillBonding&&assessment?.manualRequestAllowed===true;
  const canProceedToImage=approvedOwner||Boolean(evidence?.signedWalletMatchesAuthority&&securityPass&&autoCleared&&(!item||approvedOwner));
  ...
  const register=async()=>{
    if(!lookupComplete||!connected||working||wrongAuthorityWallet||conflictingVerified||suspended||!evidence?.signedWalletMatchesAuthority||!securityPass||!autoCleared||stillBonding||!imageFile)return;
  ...
  const claim=async()=>{
    if(!lookupComplete||!item||wrongAuthorityWallet||conflictingVerified||suspended||!evidence?.signedWalletMatchesAuthority||!securityPass||!autoCleared||stillBonding||working)return;
    setWorking(true);setFeedback(null);
  ...
  const body = <>
    ...
    {stillBonding?<section role="alert" data-import-bonding="true" className="rounded border border-amber-400/40 p-4"><h2 className="font-retro text-sm">STILL BONDING ON PUMP.FUN</h2><p className="mt-2 text-sm">This token has not completed its bonding phase. New imports for the post-grad system must wait until graduation and market verification.</p>{approvedOwner?<p className="mt-2 text-sm">Your existing project page is retained. This does not grant Battle or trading access.</p>:null}<Button className="mt-3" variant="outline" disabled={working} onClick={()=>void resolve()}>CHECK AGAIN</Button></section>:null}
