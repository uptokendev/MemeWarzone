...
export function ProjectImportPanel({
  ...
  const body = <>
    ...
    {lookupComplete?<section className={embedded ? "border-t border-white/10 pt-5" : "mwz-hud-frame p-5"} data-import-ownership-result="true">
      ...
      {canRequestManual?<div className="mt-4"><Button type="button" variant="outline" disabled={working||!imageFile} onClick={()=>void manual()}>REQUEST MANUAL CHECK</Button><p className="mt-2 text-xs text-muted-foreground">Add an image, then send the token to our team. For Pump.fun creator-wallet mismatches, we can verify you through the project's official account. Other known wallet mismatches stay blocked.</p></div>:null}
      ...
      {!approvedOwner&&!stillBonding&&autoCleared&&!wrongAuthorityWallet&&!ownershipUnavailable&&securityPass&&!item&&evidence?.signedWalletMatchesAuthority?<><h2 className="mt-4 font-retro text-sm text-emerald-200">OWNER + SECURITY VERIFIED</h2><p className="mt-2 text-sm text-muted-foreground">All automatic checks passed and this wallet matches the token owner. Add the project image to finish.</p><Button type="button" className="mt-4" disabled={!imageFile||working} onClick={()=>void register()}>REGISTER MEMECOIN</Button></>:null}
    </section>:null}
  </>;
