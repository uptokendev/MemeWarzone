...
export function ProjectImportPanel({
  ...
  const body = <>
    ...
    {lookupComplete?<section className={embedded ? "border-t border-white/10 pt-5" : "mwz-hud-frame p-5"} data-import-ownership-result="true">{wrongAuthorityWallet?<div className="mb-5 rounded-md border border-amber-300/40 bg-amber-400/10 p-4" data-import-wrong-wallet-warning="true"><h2 className="font-retro text-sm text-amber-100">WRONG WALLET CONNECTED</h2><p className="mt-2 text-sm text-amber-50">This memecoin is controlled by wallet {expectedAuthorityShort}. Connect that wallet to verify ownership.</p>{connectedWalletShort?<p className="mt-2 text-xs text-amber-100/80">Connected: {connectedWalletShort}</p>:null}</div>:null}{item?<><div className="flex items-start gap-3">{ownerVerified?<ShieldCheck className="mt-0.5 h-5 w-5 text-emerald-300"/>:<ShieldQuestion className="mt-0.5 h-5 w-5 text-amber-200"/>}<div><h2 className="font-retro text-sm text-foreground">{ownerVerified?"OWNER VERIFIED":item.ownershipStatus==="ownership_manual_review"?"OWNERSHIP REVIEW REQUESTED":evidence?.automaticOwnershipAvailable===false?"AUTOMATIC OWNERSHIP VERIFICATION UNAVAILABLE":"PROJECT REGISTERED"}</h2><p className="mt-2 text-sm text-muted-foreground">{ownerVerified?"Project page control is bound to the verified project owner.":item.ownershipStatus==="ownership_manual_review"?"Your signed ownership claim is waiting for operator review. This does not grant edit rights or Arena access.":evidence?.automaticOwnershipAvailable===false?"Current chain authority cannot be verified automatically. Any properly signed claimant may request manual project ownership review.":wrongAuthorityWallet?"Reconnect with the current authority wallet shown above before claiming automatic ownership.":"Current authority exists and this connected wallet matches that authority."}</p></div></div><div className="mt-5 flex flex-wrap gap-2">{!ownerVerified&&evidence?.signedWalletMatchesAuthority?<Button type="button" variant="outline" onClick={()=>void claim()} disabled={working}>CLAIM CURRENT OWNERSHIP</Button>:null}{!ownerVerified&&item.ownershipStatus!=="ownership_manual_review"&&evidence?.automaticOwnershipAvailable===false&&!conflictingVerified?<Button type="button" variant="outline" onClick={()=>void manual()} disabled={working}>REQUEST PROJECT CLAIM</Button>:null}{item.imageUrl?<Button type="button" onClick={()=>navigate(projectUrl(item))}><CheckCircle2 className="mr-2 h-4 w-4"/>OPEN PROJECT PAGE</Button>:imageFile?<Button type="button" variant="outline" disabled={working} onClick={()=>void register()}>ATTACH REQUIRED IMAGE</Button>:<p className="text-xs text-amber-200">A project image is required before this import is complete.</p>}</div></>:<><h2 className="font-retro text-sm text-foreground">PROJECT NOT REGISTERED</h2>{evidence?<p className="mt-2 text-sm text-muted-foreground">Resolved {evidence.symbol?`$${evidence.symbol}`:"token identity"}. Register it to create the imported project page.</p>:null}<Button type="button" className="mt-4" disabled={!connected||!validAddress||working||!evidence||!imageFile} onClick={()=>void register()}>{wrongAuthorityWallet?"REGISTER MEMECOIN":<>REGISTER &amp; VERIFY MEMECOIN</>}</Button></>}</section>:null}
  </>;
