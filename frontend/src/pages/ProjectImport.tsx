...
export function ProjectImportPanel({ embedded = false, onProjectChange }: { embedded?: boolean; onProjectChange?: (item: ProjectImportItem) => void; }) {
  ...
  const body = <>
    {embedded ? null : <section className="mwz-hud-frame p-5"><div className="text-[11px] uppercase tracking-[0.2em] text-accent">Existing project onboarding</div><h1 className="mt-2 font-retro text-2xl text-foreground">IMPORT YOUR MEMECOIN</h1><p className="mt-3 max-w-2xl text-sm text-muted-foreground">Import any existing memecoin on a supported chain, regardless of where it was launched or created. MemeWarzone resolves the token and runs critical safety checks before creating the project page. Project ownership can be claimed separately later.</p></section>}
    <section className={embedded ? "space-y-5" : "mwz-hud-frame space-y-5 p-5"}>
      <div><div className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">1. Choose chain</div><div className="mt-3 flex gap-2"><Button type="button" disabled={working} variant={chain === "bnb" ? "default" : "outline"} onClick={() => { setChainChosenByUser(true); setChain("bnb"); setFeedback(null); }}>BNB</Button><Button type="button" disabled={working} variant={chain === "solana" ? "default" : "outline"} onClick={() => { setChainChosenByUser(true); setChain("solana"); setFeedback(null); }}>Solana</Button>{projectImportRobinhoodEnabled ? <Button type="button" disabled={working} variant={chain === "robinhood" ? "default" : "outline"} onClick={() => { setChainChosenByUser(true); setChain("robinhood"); setFeedback(null); }}>Robinhood</Button> : null}</div></div>
      <div><div className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">2. Wallet</div><div className="mt-3 flex flex-wrap items-center gap-3"><Button type="button" variant="outline" onClick={() => void connect()} disabled={chain === "solana" ? solanaWallet.connectingSolana : wallet.connecting}>{connected ? "WALLET CONNECTED" : chain === "solana" ? "CONNECT SOLANA WALLET" : chain === "robinhood" ? "CONNECT ROBINHOOD WALLET" : "CONNECT BNB WALLET"}</Button>{connectedWallet ? <span className="max-w-full truncate text-xs text-muted-foreground">{connectedWallet}</span> : null}</div><p className="mt-2 text-xs text-muted-foreground">Your wallet signs the import request only. It does not need to own the token.</p></div>
      <div><label htmlFor="project-import-token" className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">3. Contract Address</label><div className="mt-3 flex flex-col gap-2 sm:flex-row"><Input id="project-import-token" disabled={working} value={tokenAddress} onChange={(e) => { setTokenAddress(e.target.value); setFeedback(null); }} placeholder="Contract Address" /><Button type="button" disabled={!validAddress || !connected || working} onClick={() => void importToken()}>{working ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}IMPORT MEMECOIN</Button></div>{!connected ? <p className="mt-2 text-xs text-amber-200">Connect a wallet to submit the import.</p> : null}{tokenAddress.trim() && !validAddress ? <p role="alert" className="mt-2 text-sm text-red-200">This Contract Address is not valid for the selected chain.</p> : null}</div>
    </section>
    {feedback ? <section role="alert" data-import-error="true" className="rounded-md border border-red-300/40 bg-red-400/10 p-4"><h2 className="font-retro text-sm text-red-100">{feedback.title}</h2><p className="mt-2 text-sm text-red-50">{feedback.message}</p>{feedback.retry ? <Button type="button" variant="outline" className="mt-3" disabled={working || !validAddress || !connected} onClick={() => void importToken()}>RETRY IMPORT</Button> : null}</section> : null}
  </>;
