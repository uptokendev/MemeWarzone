...
export function ProjectImportPanel({
  ...
  const approvedOwner=Boolean(ownerVerified&&!conflictingVerified);
  const wrongAuthorityWallet=!approvedOwner&&evidence?.automaticOwnershipAvailable===true&&Boolean(evidence.currentAuthority)&&!evidence.signedWalletMatchesAuthority;
  ...
  const manualReviewMine=Boolean(manualReviewPending&&sameWallet(item?.manualClaimWallet,connectedWallet,chain==="solana"));
  const manualReviewReason=ownershipUnavailable||securityBlocked||securityReview;
  const canRequestManual=lookupComplete&&Boolean(evidence)&&!wrongAuthorityWallet&&!approvedOwner&&!manualReviewPending&&manualReviewReason;
  const canProceedToImage=approvedOwner||Boolean(evidence?.signedWalletMatchesAuthority&&securityPass&&(!item||approvedOwner));
  const canSelectImage=canProceedToImage||canRequestManual||manualReviewMine;
  ...
  const resolve=async()=>{if(!validAddress||!connected||working)return;setWorking(true);setImageFile(null);setImagePreview("");try{const token=tokenAddress.trim();const existing=await lookupProjectImport(token,chainId);const auth=await signAction("project_import_resolve",null);const resolved=await resolveProjectImport({tokenAddress:token,chainId,auth});setItem(existing);setEvidence(resolved);setLookupComplete(true);if(resolved.automaticOwnershipAvailable&&!resolved.signedWalletMatchesAuthority){toast.error("This wallet is not the owner of this token.");}else if(!resolved.automaticOwnershipAvailable){toast.error("Token ownership cannot be verified automatically. Manual review is available.");}else if(resolved.security?.status!=="pass"){toast.error("Token security checks require manual review.");}}catch(error:any){toast.error(String(error?.message||"Project resolution failed."));setItem(null);setEvidence(null);setLookupComplete(true);}finally{setWorking(false);}};
  const register=async()=>{if(!connected||!validAddress||working)return;if(!evidence?.signedWalletMatchesAuthority&&!approvedOwner){toast.error("Token ownership must be verified before importing.");return;}if(!securityPass&&!approvedOwner){toast.error("Token security checks must pass before automatic importing.");return;}if(!imageFile){toast.error("Add a project image (PNG, JPEG or WEBP) before registering.");return;}setWorking(true);const id=toast.loading("Registering project...");try{const body={operation:"create"};const auth=await signAction("project_import_create",null,body);const result=await createProjectImport({tokenAddress:tokenAddress.trim(),chainId,auth});let project=result.project;if(!project.imageUrl){try{project=await attachRegistrationImage(project,imageFile);}catch(error:any){setItem(project);setLookupComplete(true);onProjectChange?.(project);throw new Error(String(error?.message||"Project registered, but a valid PNG, JPEG or WEBP image is still required."));}}setItem(project);setLookupComplete(true);onProjectChange?.(project);toast.success("Project registered. Ownership and security checks passed.");}catch(error:any){toast.error(String(error?.message||"Could not register project."));}finally{toast.dismiss(id);setWorking(false);}};
  const claim=async()=>{if(!item||!evidence?.signedWalletMatchesAuthority||!securityPass||working)return;setWorking(true);const id=toast.loading("Verifying current ownership...");try{const body={operation:"claim"};const auth=await signAction("project_import_claim",item.id,body);const next=await claimProjectImport({item,auth});setItem(next);onProjectChange?.(next);toast.success("Project ownership verified. You can now upload the project image.");}catch(error:any){toast.error(String(error?.message||"Could not verify ownership."));}finally{toast.dismiss(id);setWorking(false);}};
  const uploadPendingImage=async(project:ProjectImportItem)=>{if(project.imageUrl)return project;if(!imageFile)throw new Error("Add a project image before requesting manual review.");return attachRegistrationImage(project,imageFile);};
  const manual=async()=>{if(!canRequestManual||working)return;if(!imageFile){toast.error("Add a project image before requesting manual review.");return;}setWorking(true);const id=toast.loading("Requesting manual check...");try{const note=null;const auth=await signAction("project_import_manual_claim",item?.id||null,{note});let next=await requestProjectManualCheck({chainId,tokenAddress:tokenAddress.trim(),auth,note:undefined});next=await uploadPendingImage(next);setItem(next);onProjectChange?.(next);toast.success("Manual check requested. Your image is attached and the project stays hidden until an admin approves it.");}catch(error:any){toast.error(String(error?.message||"Could not complete manual check request."));}finally{toast.dismiss(id);setWorking(false);}};
  const attachManualImage=async()=>{if(!item||!manualReviewMine||item.imageUrl||!imageFile||working)return;setWorking(true);const id=toast.loading("Attaching image...");try{const next=await uploadPendingImage(item);setItem(next);onProjectChange?.(next);toast.success("Image attached to the manual review request.");}catch(error:any){toast.error(String(error?.message||"Could not attach image."));}finally{toast.dismiss(id);setWorking(false);}};
  ...
  const body = <>
    ...
    {lookupComplete?<section className={embedded ? "border-t border-white/10 pt-5" : "mwz-hud-frame p-5"} data-import-ownership-result="true">
      ...
      {manualReviewPending?<div className="mt-4 rounded-md border border-sky-300/40 bg-sky-400/10 p-4" data-import-manual-review-pending="true"><h2 className="font-retro text-sm text-sky-100">MANUAL CHECK REQUESTED</h2><p className="mt-2 text-sm text-sky-50">Your request is in the web-dashboard review queue. The project stays hidden until an admin approves it.{item?.imageUrl?" The project image is attached.":" An image is still required before approval."}</p>{manualReviewMine&&!item?.imageUrl&&imageFile?<Button type="button" className="mt-4" variant="outline" disabled={working} onClick={()=>void attachManualImage()}>ATTACH IMAGE TO REVIEW</Button>:null}</div>:null}
