...
export function ProjectImportPanel({ embedded = false, onProjectChange }: { embedded?: boolean; onProjectChange?: (item: ProjectImportItem) => void; }) {
  ...
  const body = <>
    {embedded ? null : <section className="mwz-hud-frame p-5"><div className="text-[11px] uppercase tracking-[0.2em] text-accent">Existing project onboarding</div><h1 className="mt-2 font-retro text-2xl text-foreground">IMPORT YOUR MEMECOIN</h1><p className="mt-3 max-w-2xl text-sm text-muted-foreground">Import any supported memecoin. MemeWarzone verifies the token, confirms it is no longer bonding and runs critical safety checks before creating the page. Project ownership can be claimed separately later.</p></section>}
