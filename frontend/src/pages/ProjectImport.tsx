...
export function ProjectImportPanel({
  ...
  const body = <>
    ...
    <section className={embedded ? "space-y-5" : "mwz-hud-frame space-y-5 p-5"}>
      <div><div className="text-xs font-semibold uppercase tracking-[0.14em] text-white/70">1. Choose chain</div><div className="mt-3 flex gap-2"><Button type="button" variant={chain==="bnb"?"default":"outline"} onClick={()=>{setChain("bnb");reset();}}>BNB</Button><Button type="button" variant={chain==="solana"?"default":"outline"} onClick={()=>{setChain("solana");reset();}}>Solana</Button></div></div>
    ...
    {lookupComplete?<section className={embedded ? "border-t border-white/10 pt-5" : "mwz-hud-frame p-5"} data-import-ownership-result="true">{item?<><div className="flex items-start gap-3">{ownerVerified?<ShieldCheck className="mt-0.5 h-5 w-5 text-emerald-300"/>:<ShieldQuestion className="mt-0.5 h-5 w-5 text-amber-200"/>}<div><h2 className="font-retro text-sm text-foreground">{ownerVerified?"OWNER VERIFIED":evidence?.automaticOwnershipAvailable===false?"AUTOMATIC OWNERSHIP VERIFICATION UNAVAILABLE":"PROJECT REGISTERED"}</h2><p className="mt-2 text-sm text-muted-foreground">{ownerVerified?"Project page control is bound to the verified project owner.":evidence?.automaticOwnershipAvailable===false?"Current chain authority cannot be verified automatically. Any properly signed claimant may request manual project ownership review.":"Current authority exists, but this connected wallet is not verified as that authority."}</p></div></div><div className="mt-5 flex flex-wrap gap-2">{!ownerVerified&&evidence?.signedWalletMatchesAuthority?<Button type="button" variant="outline" onClick={()=>void claim()} disabled={working}>CLAIM CURRENT OWNERSHIP</Button>:null}{!ownerVerified&&evidence?.automaticOwnershipAvailable===false&&!conflictingVerified?<Button type="button" variant="outline" onClick={()=>void manual()} disabled={working||Boolean(item.manualClaimRequestedAt)}>{item.manualClaimRequestedAt?"PROJECT CLAIM REQUESTED":"REQUEST PROJECT CLAIM"}</Button>:null}{item.imageUrl?<Button type="button" onClick={()=>navigate(projectUrl(item))}><CheckCircle2 className="mr-2 h-4 w-4"/>OPEN PROJECT PAGE</Button>:imageFile?<Button type="button" variant="outline" disabled={working} onClick={()=>void register()}>ATTACH REQUIRED IMAGE</Button>:<p className="text-xs text-amber-200">A project image is required before this import is complete.</p>}</div></>:<><h2 className="font-retro text-sm text-foreground">PROJECT NOT REGISTERED</h2>{evidence?<p className="mt-2 text-sm text-muted-foreground">Resolved {evidence.symbol?`$${evidence.symbol}`:"token identity"}. Register it to create the imported project page.</p>:null}<Button type="button" className="mt-4" disabled={!connected||!validAddress||working||!evidence||!imageFile} onClick={()=>void register()}>REGISTER &amp; VERIFY PROJECT</Button></>}</section>:null}
  </>;
