...
export function ProjectImportPanel({
  ...
  const canProceedToImage=Boolean(evidence?.signedWalletMatchesAuthority)&&securityPass&&(!item||(ownerVerified&&!conflictingVerified));
  const manualReviewPending=item?.ownershipStatus==="ownership_manual_review";
  const canRequestManual=lookupComplete&&Boolean(evidence)&&!ownerVerified&&!manualReviewPending;
  ...
  const manual=async()=>{if(!canRequestManual||working)return;setWorking(true);const id=toast.loading("Requesting manual check...");try{const note=null;const auth=await signAction("project_import_manual_claim",item?.id||null,{note});const next=await requestProjectManualCheck({chainId,tokenAddress:tokenAddress.trim(),auth,note:undefined});setItem(next);onProjectChange?.(next);toast.success("Manual check requested. Import stays locked until an admin approves it.");}catch(error:any){toast.error(String(error?.message||"Could not request manual check."));}finally{toast.dismiss(id);setWorking(false);}};
