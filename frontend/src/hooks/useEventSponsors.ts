...
export function tournamentSponsorEventType(event: Record<string, unknown> | null | undefined): SponsorEventType | null {
  const origin = String(event?.origin || "").toLowerCase();
  // Historical Quarter Finals are compatibility data, never the canonical Quarterly sponsorship target.
  if (origin === "quarter_finals") return null;
...
export function useEventSponsors(input: {
  ...
  const enabled = input.enabled !== false && Boolean(input.eventType) && Boolean(String(input.eventReferenceId || "").trim());

  useEffect(() => {
    if (!enabled || !input.eventType) {
      setSponsors([]);
      return;
    }
    const controller = new AbortController();
    fetchEventSponsors({ eventType: input.eventType, eventReferenceId: input.eventReferenceId, chainId: input.chainId, signal: controller.signal })
