...
export type CreateCampaignInstructionArgs = {
  ...
  metadataHash: number[];
  /**
   * Metaplex name and symbol. Required again in v7: create_campaign writes the
   * metadata itself, so these are what the token is called.
   */
  name: string;
  symbol: string;
  launchAt: string;
  ...
  deadline: string;
  /**
   * v7 dropped clusterHash, tickerHash, reservationIdHash, reservationVersion
   * and nonce. Each was 32 bytes of a transaction with none to spare and none
   * was ever read back: clusterHash was an env constant, tickerHash is derived
   * in-program from the symbol, campaignId is already derived from
   * reservationIdHash, and nonce only seeded an account v7 does not create.
   * The API may still send them; they are ignored here.
   */
  clusterHash?: number[];
  tickerHash?: number[];
...
export type CreateCampaignInstructionAccounts = {
  ...
  tokenVault: string;
  solVault: string;
  /** v7 writes metadata and the fee accounts inside create. */
  ...
  /** v7 does not create this account. Accepted and ignored. */
...
export function encodeCreateCampaignData(args: CreateCampaignInstructionArgs): Uint8Array {
  // Must match CreateCampaignArgs in authorized_create.rs field for field.
  return concatBytes([
...
export function buildCreateCampaignInstruction(
  ...
  return new TransactionInstruction({
    ...
    keys: [
      ...
      // Read-only. The program only reads route_signer from it; marking it
      // writable made Phantom attach a 41-byte Lighthouse assertion, the single
      // most expensive item in the transaction.
