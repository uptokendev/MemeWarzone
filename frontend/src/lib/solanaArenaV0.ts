...
function assertPoolId(poolId: Uint8Array): Uint8Array {
  if (!(poolId instanceof Uint8Array) || poolId.length !== 32) {
    throw new Error("Arena pool id must be exactly 32 bytes.");
  }
  return poolId;
}
...
function canonicalProgramId(): string {
  ...
  if (configured !== REWARDS_TREASURY_PROGRAM_ID) {
    throw new Error(
      `Arena refuses non-canonical rewards treasury program: ${configured || "missing"}`,
    );
  }
  return configured;
}
...
export function deriveArenaPdas(web3: SolanaWeb3Module, poolId: Uint8Array): ArenaPdas {
  ...
  const config = derivePda(web3, programId, [utf8(ARENA_CONFIG_SEED)]);
  const pool = derivePda(web3, programId, [utf8(ARENA_POOL_SEED), canonicalPoolId]);
  const vault = derivePda(web3, programId, [utf8(ARENA_VAULT_SEED), canonicalPoolId]);
  return {
...
export function deriveArenaBuyInReceipt(
  ...
  const canonicalPoolId = assertPoolId(poolId);
  const programId = new web3.PublicKey(canonicalProgramId());
  const entrantKey = new web3.PublicKey(entrant);
...
export function deriveArenaClaimReceipt(
  web3: SolanaWeb3Module,
  poolId: Uint8Array,
  bucket: number,
): string {
  if (!Number.isInteger(bucket) || bucket < 0 || bucket > 255) {
    throw new Error("Invalid Arena claim bucket.");
  }
  const canonicalPoolId = assertPoolId(poolId);
  const programId = new web3.PublicKey(canonicalProgramId());
  return derivePda(web3, programId, [
    utf8(ARENA_CLAIM_SEED),
    canonicalPoolId,
    Uint8Array.of(bucket),
  ]).toBase58();
}

export function deriveArenaRefundReceipt(
  web3: SolanaWeb3Module,
  poolId: Uint8Array,
  wallet: string,
): string {
...
export async function buildArenaOpenBattleV0Instruction(input: {
  ...
  ownerA: string;
  ownerB: string;
  stakeLamports: number | string | bigint;
  ...
  const programId = new input.web3.PublicKey(pdas.programId);
  ...
  const data = concat(
    await anchorDiscriminator("open_battle_pool"),
    poolId,
    ownerA.toBytes(),
    ownerB.toBytes(),
    u64le(input.stakeLamports),
    i64le(input.depositDeadline),
    i64le(input.resolveDeadline),
  );
  return {
    pdas,
...
export async function buildArenaDepositStakeV0Instruction(input: {
  ...
  return {
    ...
    instruction: new input.web3.TransactionInstruction({
      ...
      data: concat(await anchorDiscriminator("deposit_stake"), poolId),
    }),
  };
}
...
export async function buildArenaSupportV0Instruction(input: {
  ...
  return {
    ...
    instruction: new input.web3.TransactionInstruction({
      ...
      data: concat(
        await anchorDiscriminator("donate_support"),
        poolId,
...
export async function buildArenaBuyInV0Instruction(input: {
  ...
  const poolId = assertPoolId(input.poolId);
  const pdas = deriveArenaPdas(input.web3, poolId);
  const receipt = deriveArenaBuyInReceipt(input.web3, poolId, input.entrant);
  return {
    pdas,
    receipt,
    instruction: new input.web3.TransactionInstruction({
      ...
      data: concat(await anchorDiscriminator("deposit_buy_in"), poolId),
    }),
  };
}
...
export async function buildArenaStakeRefundV0Instruction(input: {
  ...
  const poolId = assertPoolId(input.poolId);
  const pdas = deriveArenaPdas(input.web3, poolId);
  const receipt = deriveArenaRefundReceipt(input.web3, poolId, input.staker);
  return {
    pdas,
    receipt,
    instruction: new input.web3.TransactionInstruction({
...
export async function buildArenaBuyInRefundV0Instruction(input: {
  ...
  const poolId = assertPoolId(input.poolId);
  const pdas = deriveArenaPdas(input.web3, poolId);
  const buyInReceipt = deriveArenaBuyInReceipt(input.web3, poolId, input.entrant);
  const refundReceipt = deriveArenaRefundReceipt(input.web3, poolId, input.entrant);
  return {
    pdas,
    ...
    instruction: new input.web3.TransactionInstruction({
      ...
      data: concat(await anchorDiscriminator("refund_buy_in"), poolId),
    }),
  };
}
...
/**
 * Common Arena user-send path. Every money-moving Arena user action must use this
 * rather than constructing a legacy Transaction directly in a React component.
 */
export async function submitArenaUserV0(input: {
  ...
  const simulated = await compileSolanaUserV0WithLatestBlockhash(
    input.web3,
    input.connection,
    intent,
  );
  ...
  const confirmation = await confirmLaunchpadSignature(input.connection, {
    ...
    recover: input.recoveryReceipt
      ? () => accountExists(input.web3, input.connection, input.recoveryReceipt!)
      : undefined,
  });
  if (confirmation.err) {
