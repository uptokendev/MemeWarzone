...
pub struct GraduationState {
    ...
    pub max_deviation_bps: u16,
    pub quote_recovery_account: Pubkey,
    pub authority_lamports_before: u64,
...
pub fn begin_graduation_handler(
    ...
    let authority_lamports_before = ctx.accounts.authority.lamports();
    ...
    state.authority_lamports_before = authority_lamports_before;
...
pub fn confirm_graduation_handler(ctx: Context<ConfirmGraduation>) -> Result<()> {
    ...
    if !native_quote {
        require!(
            ctx.accounts.authority.lamports() <= state.authority_lamports_before,
            LaunchpadError::GraduationAssetMismatch
        );
    }
    if native_quote {
...
fn validate_quote_pool_deviation(
    ...
    let expected = final_spot_nano
        .checked_mul(u128::from(sol_usd_micros))
        .ok_or(LaunchpadError::MathOverflow)?
        .checked_div(u128::from(quote_usd_micros))
        .ok_or(LaunchpadError::MathOverflow)?
