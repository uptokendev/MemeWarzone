...
function required(name: string): string { const value = String(process.env[name] || "").trim(); if (!value) fail(`${name} is required`); return value; }
...
function tokenProgramFromFlag(flag: unknown): PublicKey { return Number(flag ?? 0) === 0 ? TOKEN_PROGRAM_ID : TOKEN_2022_PROGRAM_ID; }
...
async function balanceForAsset(connection: Connection, owner: PublicKey, mint: PublicKey, tokenProgram: PublicKey): Promise<bigint> {
  if (mint.equals(WSOL)) return BigInt(await connection.getBalance(owner, "confirmed"));
  try {
    const response = await connection.getTokenAccountBalance(deriveAta(owner, mint, tokenProgram), "confirmed");
...
async function snapshot(connection: Connection, owner: PublicKey, mintA: PublicKey, mintB: PublicKey, tokenAProgram: PublicKey, tokenBProgram: PublicKey) {
  const [a, b] = await Promise.all([
    balanceForAsset(connection, owner, mintA, tokenAProgram),
    balanceForAsset(connection, owner, mintB, tokenBProgram),
  ]);
  return { a, b };
}
function delta(after: { a: bigint; b: bigint }, before: { a: bigint; b: bigint }) { return { a: after.a - before.a, b: after.b - before.b }; }

function assertEvidence(input: { chainId: number; campaign: string; mint: string; creator: string; pool: string; position: string }, canonical: { campaign: string; mint: string; creator: string; pool: string; position: string }) {
...
function expectRejected(label: string, fn: () => void) { let rejected = false; try { fn(); } catch { rejected = true; } if (!rejected) fail(`${label} proof was not rejected`); }
...
async function ensureCertificationCampaign(pool: Pool, canonical: any) {
  await pool.query(`create table if not exists public.campaigns (
    chain_id integer not null, campaign_address text not null, token_address text, creator_address text,
    name text, symbol text, graduated_at_chain timestamptz, meta jsonb not null default '{}'::jsonb,
...
function receiverAccount(owner: PublicKey, mint: PublicKey, tokenProgram: PublicKey): string {
  return mint.equals(WSOL) ? owner.toBase58() : deriveAta(owner, mint, tokenProgram).toBase58();
}
...
async function execute() {
  ...
  const campaignInfo = await connection.getAccountInfo(new PublicKey(pre.campaign), "confirmed");
  ...
  const canonical = { campaign: pre.campaign, mint: campaignState.mint.toBase58(), creator: campaignState.creator.toBase58(), pool: grad.meteoraPool, position: grad.meteoraPosition };
  ...
  const [poolInfo, positionInfo, poolState, positionState] = await Promise.all([
    connection.getAccountInfo(poolPk, "confirmed"), connection.getAccountInfo(positionPk, "confirmed"), cpAmm.fetchPoolState(poolPk), cpAmm.fetchPositionState(positionPk),
  ]);
  ...
  const pair = [mintA.toBase58(), mintB.toBase58()];
  ...
  if (!listedBefore.items?.find((item: any) => item.campaignAddress === canonical.campaign)?.fees?.registered) fail("runtime LP fee reader did not register exact campaign position");
  ...
  if (bi(harvest?.grossClaimRaw?.tokenA) !== feeA || bi(harvest?.grossClaimRaw?.tokenB) !== feeB) {
    fail(`runtime gross claim mismatch expected=${feeA}/${feeB} actual=${harvest?.grossClaimRaw?.tokenA}/${harvest?.grossClaimRaw?.tokenB}`);
  }
  if (harvest?.sourceVaultEvidence?.tokenA?.address !== poolState.tokenAVault.toBase58() || harvest?.sourceVaultEvidence?.tokenB?.address !== poolState.tokenBVault.toBase58()) fail("runtime gross claim did not bind the exact Meteora source vaults");
  if (bi(harvest?.sourceVaultEvidence?.tokenA?.before) - bi(harvest?.sourceVaultEvidence?.tokenA?.after) !== feeA) fail("token A source-vault movement differs from claimable gross fee");
  if (bi(harvest?.sourceVaultEvidence?.tokenB?.before) - bi(harvest?.sourceVaultEvidence?.tokenB?.after) !== feeB) fail("token B source-vault movement differs from claimable gross fee");
  ...
  if (creatorDelta.a !== expectedA.creator || creatorDelta.b !== expectedB.creator) fail(`creator receiver delta mismatch expected=${expectedA.creator}/${expectedB.creator} actual=${creatorDelta.a}/${creatorDelta.b}`);
  if (treasuryDelta.a !== expectedA.protocol || treasuryDelta.b !== expectedB.protocol) fail(`protocol receiver delta mismatch expected=${expectedA.protocol}/${expectedB.protocol} actual=${treasuryDelta.a}/${treasuryDelta.b}`);
  ...
  if (bi(dbHarvest.grossARaw) !== feeA || bi(dbHarvest.grossBRaw) !== feeB) fail("DB reconciliation did not preserve gross claim amounts");

  const [retryCreatorBefore, retryTreasuryBefore] = await Promise.all([
    snapshot(connection, creatorPk, mintA, mintB, tokenAProgram, tokenBProgram), snapshot(connection, treasury, mintA, mintB, tokenAProgram, tokenBProgram),
  ]);
  ...
  if ([retryCreatorDelta.a, retryCreatorDelta.b, retryTreasuryDelta.a, retryTreasuryDelta.b].some((value) => value !== 0n)) fail("retry caused a duplicate receiver transfer");
  ...
  const report = {
    ...
    grossClaimRaw: { tokenA: feeA.toString(), tokenB: feeB.toString() },
    sourceVaultEvidence: harvest.sourceVaultEvidence,
    ...
    receiverAccounts: {
      creator: { tokenA: receiverAccount(creatorPk, mintA, tokenAProgram), tokenB: receiverAccount(creatorPk, mintB, tokenBProgram) },
      protocol: { tokenA: receiverAccount(treasury, mintA, tokenAProgram), tokenB: receiverAccount(treasury, mintB, tokenBProgram) },
    },
    settlementMode: harvest.settlementMode,
    operatorTransactionCostsLamports: { claim: String(claimTx.meta?.fee || 0), split: String(splitTx.meta?.fee || 0) },
    ...
    transactionPrograms: { claim: "Meteora CP-AMM claimPositionFee with authoritative source-vault accounting", split: "native SOL for WSOL side; SPL transfer for campaign-token side" },
    dbReconciliation: { claimTx: dbHarvest.claimTx, splitTx: dbHarvest.splitTx, lastTx: dbHarvest.lastTx, grossARaw: dbHarvest.grossARaw, grossBRaw: dbHarvest.grossBRaw, status: "PASS" },
    ...
    transactionFeesAreOperatorCosts: true,
...
async function reload() {
  ...
  if (String(row.harvest?.grossARaw || "") !== String(report.grossClaimRaw.tokenA) || String(row.harvest?.grossBRaw || "") !== String(report.grossClaimRaw.tokenB)) fail("fresh-process reload did not preserve gross claim reconciliation");
  console.log(JSON.stringify({ status: "PASS", reload: "fresh-process", chainId: CHAIN_ID, campaign: report.campaign, claimTx: row.harvest.claimTx, splitTx: row.harvest.splitTx, grossARaw: row.harvest.grossARaw, grossBRaw: row.harvest.grossBRaw }, null, 2));
}
