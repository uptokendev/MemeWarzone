        ...
        await client.query(
          `update public.campaign_market_state
              set market_stage='DEX_ACTIVE',dex_router_address=$3,dex_factory_address=$4,
                  wrapped_native_address=$5,pool_stable=false,pool_fee_bps=$6,pool_verified=true,
...
async function upsertCandle(input: { indexedPool: IndexedPool; blockTime: Date; blockNumber: number; logIndex: number; priceNative: string; nativeAmountRaw: bigint }): Promise<void> {
  ...
  for (const resolution of Object.keys(RESOLUTION_MS) as CandleResolution[]) {
    await pool.query(
      ...
      [input.indexedPool.chainId,input.indexedPool.campaignAddress,resolution,bucketStart(input.blockTime, resolution),input.priceNative,volumeNative,input.blockNumber,input.logIndex],
    );
  }
}
...
async function insertSwap(provider: ethers.JsonRpcProvider,indexedPool: IndexedPool,log: ethers.Log,_parsed: ethers.LogDescription,normalized: NormalizedSwap): Promise<boolean> {
  ...
  const inserted = await pool.query(
    `insert into public.dex_trades(chain_id,campaign_address,token_address,pair_address,tx_hash,log_index,block_number,block_hash,block_time,status,side,sender_address,recipient_address,transaction_from,token_amount_raw,native_amount_raw,token_amount,native_amount,price_bnb,execution_source,origin,trade_intent_id,created_at,updated_at)
  ...
  await pool.query(
    `insert into public.market_stats(chain_id,campaign_address,market_stage,last_price_bnb,dex_volume_24h_bnb,volume_24h_bnb,trades_24h,buys_24h,sells_24h,last_trade_block,last_trade_at,updated_at)
     values($1,$2,'DEX_ACTIVE',$3,$4,$4,1,$5,$6,$7,$8,now())
     on conflict(chain_id,campaign_address) do update set market_stage='DEX_ACTIVE',last_price_bnb=excluded.last_price_bnb,
    ...
    [indexedPool.chainId,indexedPool.campaignAddress,priceNative,nativeAmount,normalized.side === "buy" ? 1 : 0,normalized.side === "sell" ? 1 : 0,log.blockNumber,blockTime],
  );
  await publishMarketEvent(indexedPool, "market_trade", {eventId:`${indexedPool.chainId}:${txHash}:${logIndex}`,source:"robinhood_v3",origin,side:normalized.side,wallet:transactionFrom || normalized.sender,recipient:normalized.recipient,tokenAmountRaw:normalized.tokenAmountRaw.toString(),nativeAmountRaw:normalized.nativeAmountRaw.toString(),priceBnb:priceNative,txHash,logIndex,blockNumber:log.blockNumber,blockTime:blockTime.toISOString(),status:"confirmed"});
  return true;
}
