...
export function validateBootstrapBoundary({ chainId, networkName, broadcastToken, manifestExists = false }) {
  ...
  if (Number(chainId) !== ROBINHOOD_TESTNET_CHAIN_ID || networkName !== "robinhoodTestnet") {
    throw new Error(`bootstrap requires robinhoodTestnet / ${ROBINHOOD_TESTNET_CHAIN_ID}`);
  }
...
export function requireSwapRouter02Runtime(code) {
  ...
  if (!runtime.includes(SWAP_ROUTER02_EXACT_INPUT_SINGLE_SELECTOR)) {
    throw new Error("swap router does not expose SwapRouter02 no-deadline exactInputSingle selector 0x04e45aaf");
  }
  return true;
}
...
export function buildInfrastructureManifest(input) {
  ...
  validateOracleObservation({
    decimals: input.oracle?.decimals,
    roundId: input.oracle?.roundId,
    answer: input.oracle?.answer,
    updatedAt: input.oracle?.updatedAt,
    answeredInRound: input.oracle?.answeredInRound,
    currentTimestamp: input.oracle?.certifiedAtTimestamp,
  ...
  return {
    ...
    oracle: {
      ...input.deployments.oracle,
      updater: oracleUpdater,
      decimals: Number(input.oracle.decimals),
      lastCertifiedRound: String(input.oracle.roundId),
      lastCertifiedAnswer: String(input.oracle.answer),
      lastCertifiedTimestamp: Number(input.oracle.updatedAt),
      answeredInRound: String(input.oracle.answeredInRound),
      maxAgeSeconds: Number(input.oracle.maxAgeSeconds),
    },
    bindings: {
      npmFactory: input.bindings.npmFactory,
      npmWeth9: input.bindings.npmWeth9,
