...
export function liveRequested(env = process.env) {
  return String(env.RH46630_DEPLOY_REWARD_DISTRIBUTOR || '').trim() === '1';
}

export function planRewardDistributorDeploy(input = {}, env = process.env) {
  ...
  return {
    ...
    constructor: { initialOwner: OWNER },
    note: 'Merkle generic claims stay REWARD_CLAIMS_ENABLED=false until funded + root. 4663 forbidden.',
    liveRequested: live,
    sendRequired: live,
  };
}

export async function runRewardDistributorDeploy({ env = process.env, plan, sendDeploy } = {}) {
  const resolved = plan || planRewardDistributorDeploy({}, env);
...
function loadArtifact() {
  const artifactPath = path.resolve('artifacts/contracts/RewardDistributor.sol/RewardDistributor.json');
  if (!fs.existsSync(artifactPath)) throw new Error('MISSING_REWARD_DISTRIBUTOR_ARTIFACT');
  return JSON.parse(fs.readFileSync(artifactPath, 'utf8'));
}

async function main() {
  const plan = planRewardDistributorDeploy({}, process.env);
  ...
  const key = String(process.env.RH46630_DISTRIBUTOR_OWNER_PRIVATE_KEY || process.env.ROBINHOOD_TESTNET_DEPLOYER_PRIVATE_KEY || '').trim();
  ...
  const deployed = new ethers.Contract(address, ['function owner() view returns (address)'], provider);
...
if (invokedPath === import.meta.url) {
  main().catch((error) => {
    console.error(`[rh46630-reward-distributor-deploy] ${error?.stack || error}`);
    process.exitCode = 1;
  });
}
