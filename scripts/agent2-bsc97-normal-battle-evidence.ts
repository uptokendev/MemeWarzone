...
async function createAuth(mod: any, factory: any, creator: ethers.Wallet, routeAuthority: ethers.Wallet, request: any) {
  ...
  const signature = await mod.signCreateAuthorization({ signer: routeAuthority, chainId: CHAIN_ID, factoryAddress: await factory.getAddress(), creator: creator.address, request, tradeRouteProfileId: tradeRouteProfile, finalizeRouteProfileId: finalizeRouteProfile, deadline });
...
async function tradeAuth(mod: any, campaign: any, actor: ethers.Wallet, routeAuthority: ethers.Wallet, action: number, amount: bigint, limit: bigint) {
  ...
  const deadline = (await latestTimestamp()) + 3600n;
  const signature = await mod.signTradeAuthorization({ signer: routeAuthority, chainId: CHAIN_ID, campaignAddress: await campaign.getAddress(), actor: actor.address, routeProfileId, action, amount, limit, deadline });
  return { routeProfileId, deadline, signature };
}
async function executeSide(label: string, factory: any, mod: any, routeAuthority: ethers.Wallet, creator: ethers.Wallet, buyer: ethers.Wallet, trader: ethers.Wallet) {
  const now = Date.now();
  const request = { name: `Agent2 ${label} ${now}`, symbol: `A2${label}${String(now).slice(-4)}`.slice(0, 10), logoURI: "ipfs://agent2-normal-battle-cert", xAccount: "", website: "", extraLink: "", graduationTarget: ethers.parseEther("6") };
  ...
  const holdersBefore = { buyer: (await token.balanceOf(buyer.address)).toString(), trader: (await token.balanceOf(trader.address)).toString() };
  ...
  const startMcapNativeWei = (quoteBefore * supply) / unit;
  const endMcapNativeWei = (quoteAfter * supply) / unit;
  return {
    label, chainId: CHAIN_ID, campaign: info.campaign, token: info.token, creator: creator.address, buyer: buyer.address, trader: trader.address,
    totalSupply: supply.toString(), quoteProbeTokenRaw: unit.toString(), quoteProbeBeforeWei: quoteBefore.toString(), quoteProbeAfterWei: quoteAfter.toString(),
    startMcapNativeWei: startMcapNativeWei.toString(), marginalMcapNativeWei: endMcapNativeWei.toString(), holdersBefore, holdersAfter, holderCountDeltaKnown: 2,
    transactions: {
      create: { hash: createTx.hash, blockNumber: createReceipt?.blockNumber },
      buyBuyer: { hash: buy1Tx.hash, blockNumber: buy1Receipt?.blockNumber, nativeWei: buy1Cost.toString(), tokens: buy1Amount.toString(), wallet: buyer.address },
      buyTrader: { hash: buy2Tx.hash, blockNumber: buy2Receipt?.blockNumber, nativeWei: buy2Cost.toString(), tokens: buy2Amount.toString(), wallet: trader.address },
      sellBuyer: { hash: sellTx.hash, blockNumber: sellReceipt?.blockNumber, nativeWei: sellPayout.toString(), tokens: sellAmount.toString(), wallet: buyer.address },
...
async function main() {
  ...
  const deployerKey = String(process.env.BSC_TESTNET_PRIVATE_KEY || process.env.DEPLOYER_PK || process.env.PRIVATE_KEY_DEPLOY || "").trim();
  ...
  const report = { schemaVersion: 2, purpose: "agent2-normal-battle-destructive-market-evidence", sourceSha: process.env.GITHUB_SHA || null, chainId: CHAIN_ID, network: "bsc-testnet", nativeUsdPrice: Number(manifest.testNativeUsdPrice || 3000), factory: await factory.getAddress(), manifestEvidence: manifest.evidenceSource || null, left, right, checks: { freshCampaigns: true, chainDerivedMcapGrowth: true, holderGrowth: true, eligibleRealVolumeTransactions: 6, crossTokenIsolation: true, claimsTouched: false, graduationTouched: false } };
