...
test("accepts a complete dark 4663 preflight isolated from accepted testnet", () => {
  const result = proveRobinhoodProductionManifest(validManifest(), { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA });
...
test("current committed mainnet placeholder cannot masquerade as a deployed production preflight", () => {
  ...
  assert.throws(() => proveRobinhoodProductionManifest(placeholder, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }), /deploymentBlock|generation|sourceSha|contract|oracle/i);
});
...
test("rejects a production manifest that reuses an accepted 46630 address", () => {
  ...
  assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: testnet, candidateSha: CANDIDATE_SHA }), /reuses accepted Robinhood testnet address/i);
});
...
test("rejects wrong chain, generation, source SHA, duplicate contracts, or missing required contract", () => {
  for (const mutate of [
    ...
    assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }));
  }
});

test("requires production route authority separation", () => {
  const manifest = validManifest(); manifest.routeAuthority = manifest.admin;
  assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }), /distinct from admin/i);
});

test("rejects any attempt to enable creation or Stock product surfaces before canary", () => {
  for (const flag of ["supportEnabled","creationEnabled","stockMarketsEnabled","stockGraduationEnabled","stockEthRoutingEnabled","stockMarketUiEnabled","beatTheMarketEnabled"]) {
    const manifest = validManifest(); manifest[flag] = true;
    assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }), new RegExp(`${flag}=false`, "i"));
  }
});
...
test("rejects duplicate canonical Stock symbols or token identities", () => {
  for (const duplicateField of ["symbol", "contractAddress"]) {
    ...
    assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }), /duplicates/i);
  }
});

test("rejects incomplete oracle, route, canary, or rollback activation prerequisites", () => {
  for (const prerequisites of [["oracle", "route", "canary"],["oracle", "route", "rollback", "manual review"]]) {
    const manifest = validManifest(); manifest.activationPrerequisites = prerequisites;
    assert.throws(() => proveRobinhoodProductionManifest(manifest, { acceptedTestnet: acceptedTestnet(), candidateSha: CANDIDATE_SHA }));
  }
});
