...
const DARK_FLAGS = ["supportEnabled","creationEnabled","stockMarketsEnabled","stockGraduationEnabled","stockEthRoutingEnabled","stockMarketUiEnabled","beatTheMarketEnabled"];
...
function boundedBps(value, label) {
  ...
  if (number > 10_000) throw new Error(`${label} must be <= 10000`);
  return number;
}

function proveGraduationPolicy(stock, manifestOracleMaxAgeSeconds) {
  const policy = stock.graduationPolicy || {};
  const maxOracleAgeSeconds = positiveInteger(policy.maxOracleAgeSeconds, "stock.graduationPolicy.maxOracleAgeSeconds");
  if (maxOracleAgeSeconds !== manifestOracleMaxAgeSeconds) {
    throw new Error("stock.graduationPolicy.maxOracleAgeSeconds must equal oracleMaxAgeSeconds");
  }
  return {
    maxOracleAgeSeconds,
    maxSwapSlippageBps: boundedBps(policy.maxSwapSlippageBps, "stock.graduationPolicy.maxSwapSlippageBps"),
    maxOracleDeviationBps: boundedBps(policy.maxOracleDeviationBps, "stock.graduationPolicy.maxOracleDeviationBps"),
    maxPriceImpactBps: boundedBps(policy.maxPriceImpactBps, "stock.graduationPolicy.maxPriceImpactBps"),
    minimumRouteLiquidityUsd: positiveNumber(policy.minimumRouteLiquidityUsd, "stock.graduationPolicy.minimumRouteLiquidityUsd"),
  };
}

function proveStockRegistry(stock, forbidden, contracts, policy) {
  if (!Array.isArray(stock.registry) || stock.registry.length === 0) throw new Error("production Stock registry must contain at least one canonical route entry");
  ...
  for (const [index, entry] of stock.registry.entries()) {
    ...
    seenSymbols.add(symbol); seenTokens.add(token);
    ...
    if (String(entry.oracleType || "").trim().toLowerCase() !== "chainlink") throw new Error(`${prefix}.oracleType must be chainlink for production acceptance`);
    ...
    if (entry.enabledForGraduation !== false || entry.enabledForTrading !== false) throw new Error(`${prefix} must keep graduation/trading disabled during production preflight`);

    const minimumQuoteLiquidityUsd = positiveNumber(entry.minimumQuoteLiquidityUsd, `${prefix}.minimumQuoteLiquidityUsd`);
    if (minimumQuoteLiquidityUsd < policy.minimumRouteLiquidityUsd) throw new Error(`${prefix}.minimumQuoteLiquidityUsd is below graduation policy minimum`);
    const maxImpactBps = boundedBps(entry.maximumGraduationSwapImpactBps, `${prefix}.maximumGraduationSwapImpactBps`);
    if (maxImpactBps > policy.maxPriceImpactBps) throw new Error(`${prefix}.maximumGraduationSwapImpactBps exceeds graduation policy`);

    const feeTier = Number(entry.acquisitionFeeTier);
    if (!Number.isInteger(feeTier) || feeTier <= 0 || feeTier > 1_000_000) throw new Error(`${prefix}.acquisitionFeeTier is invalid`);
    if (String(entry.acquisitionQuoteKind || "").trim() !== "SIMPLE_EXACT_INPUT_SINGLE") throw new Error(`${prefix}.acquisitionQuoteKind must be SIMPLE_EXACT_INPUT_SINGLE`);
    if (acquisitionRouter !== approvedRouter) throw new Error(`${prefix}.acquisitionRouterAddress must equal contracts.v3SwapRouter`);

    for (const [label, address] of [["token",token],["oracle",oracle],["acquisition pool",acquisitionPool],["acquisition quoter",acquisitionQuoter],["acquisition router",acquisitionRouter]]) {
...
export function proveRobinhoodProductionManifest(manifest, options = {}) {
  ...
  if (Number(manifest.factoryGeneration) !== EXPECTED_FACTORY_GENERATION || Number(manifest.campaignGeneration) !== EXPECTED_CAMPAIGN_GENERATION || Number(manifest.liquidityKind) !== EXPECTED_LIQUIDITY_KIND) throw new Error(`wrong production generation/liquidity metadata: expected factory ${EXPECTED_FACTORY_GENERATION} / campaign ${EXPECTED_CAMPAIGN_GENERATION} / liquidity ${EXPECTED_LIQUIDITY_KIND}`);
  ...
  const oracleMaxAgeSeconds = positiveInteger(manifest.oracleMaxAgeSeconds, "oracleMaxAgeSeconds");
  ...
  const policy = proveGraduationPolicy(stock, oracleMaxAgeSeconds);
  proveStockRegistry(stock, forbidden, contracts, policy);
...
if (runningAsCli()) {
  ...
  if (!manifestFile) { console.error("usage: node scripts/prove-robinhood-production-manifest.mjs <mainnet-manifest.json> [accepted-testnet.json] [candidate-sha]"); process.exit(2); }
