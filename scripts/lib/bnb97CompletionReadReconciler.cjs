function completionStateIsFinalized(state) {
  return Boolean(
    state &&
      state.launched === true &&
...
async function reconcileCompletionReadAfterWrite({
  receiptBlockNumber,
  receiptBlockHash,
  finalizedEvent,
  readAtBlock,
  ...
  const endpointLagObserved = !completionStateIsFinalized(latestState);
...
module.exports = { completionStateIsFinalized, reconcileCompletionReadAfterWrite };
