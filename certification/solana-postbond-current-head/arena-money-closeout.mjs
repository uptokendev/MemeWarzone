...
function readPool(info) {
  const b = body(info, 'CompetitionPoolV2');
  let o = 0;
  const generation = b[o++];
  const competitionId = b.subarray(o, o += 32).toString('hex');
  const kind = b[o++];
  const state = b[o++];
  const authority = readPub(b, o); o += 32;
  const assetA = readPub(b, o); o += 32;
  const assetB = readPub(b, o); o += 32;
  const ownerA = readPub(b, o); o += 32;
  const ownerB = readPub(b, o); o += 32;
  const requiredEntry = b.readBigUInt64LE(o); o += 8;
  const entryTotal = b.readBigUInt64LE(o); o += 8;
  const entryCount = b.readUInt32LE(o); o += 4;
  const boostGross = b.readBigUInt64LE(o); o += 8;
  const boostPrize = b.readBigUInt64LE(o); o += 8;
  const boostProtocol = b.readBigUInt64LE(o); o += 8;
  const winnerAsset = readPub(b, o); o += 32;
  const winnerWallet = readPub(b, o); o += 32;
  const pendingWinner = b.readBigUInt64LE(o); o += 8;
  const pendingLeague = b.readBigUInt64LE(o); o += 8;
  const pendingProtocol = b.readBigUInt64LE(o); o += 8;
  ...
  return {
    ...
    requiredEntry: String(requiredEntry), entryTotal: String(entryTotal), entryCount,
    boostGross: String(boostGross), boostPrize: String(boostPrize), boostProtocol: String(boostProtocol),
    winnerAsset, winnerWallet,
    pendingWinner: String(pendingWinner), pendingLeague: String(pendingLeague), pendingProtocol: String(pendingProtocol),
    winnerClaimed, leagueClaimed, protocolClaimed,
  };
}
function readConfig(info) {
  ...
  return {
    generation: b[0],
    authority: readPub(b, 1),
    resolver: readPub(b, 33),
    protocolReceiver: readPub(b, 65),
    marketingReceiver: readPub(b, 97),
...
async function reconcile(connection, signature) {
  for (let n = 0; n < 40; n += 1) {
    ...
    if (status?.err) return { state: 'failed', error: status.err, confirmationStatus: status.confirmationStatus, slot: status.slot ?? null };
    if (status?.confirmationStatus === 'confirmed' || status?.confirmationStatus === 'finalized') {
      return { state: 'landed', confirmationStatus: status.confirmationStatus, slot: status.slot ?? null };
    }
    await new Promise((resolve) => setTimeout(resolve, 500));
  }
  return { state: 'unknown', slot: null };
}
...
async function expectReject(connection, payer, instructions, signers = [payer], label = 'reject') {
  try {
    await send(connection, payer, instructions, signers);
  } catch (error) {
...
async function samePacketDuplicateProbe(connection, authority) {
  const instruction = SystemProgram.transfer({ fromPubkey: authority.publicKey, toPubkey: authority.publicKey, lamports: 1 });
  const built = await latestTx(connection, authority, [instruction], [authority]);
  const firstReturned = await connection.sendRawTransaction(built.raw, { skipPreflight: false, maxRetries: 5 });
  assert(firstReturned === built.signature, 'same-packet first send signature mismatch');
  const firstState = await reconcile(connection, built.signature);
  assert(firstState.state === 'landed', 'same-packet first send did not land');
  let secondReturned = null;
  let secondError = null;
  try {
    secondReturned = await connection.sendRawTransaction(built.raw, { skipPreflight: false, maxRetries: 0 });
    assert(secondReturned === built.signature, 'same-packet duplicate returned a different signature');
  } catch (error) {
    secondError = String(error?.message || error);
    const normalized = secondError.toLowerCase();
    assert(normalized.includes('already') || normalized.includes('processed') || normalized.includes('duplicate'), `same-packet duplicate failed unexpectedly: ${secondError}`);
  }
  const secondState = await reconcile(connection, built.signature);
  assert(secondState.state === 'landed', 'same-packet signature no longer reconciles after duplicate broadcast');
  return { signature: built.signature, firstReturned, secondReturned, secondError, firstState, secondState, identicalPacket: true, oneCanonicalSignature: true };
}

async function ambiguousResponseProbe(connection, authority) {
  const instruction = SystemProgram.transfer({ fromPubkey: authority.publicKey, toPubkey: authority.publicKey, lamports: 1 });
  const built = await latestTx(connection, authority, [instruction], [authority]);
  await connection.sendRawTransaction(built.raw, { skipPreflight: false, maxRetries: 5 });
  const reconciled = await reconcile(connection, built.signature);
  assert(reconciled.state === 'landed', 'ambiguous-response signature did not reconcile');
  return { signature: built.signature, rpcReturnIntentionallyDiscarded: true, reconciled, recoveredWithoutReplacementTransaction: true };
}

async function lostResponseClaim(connection, winner, claimInstruction, poolKey) {
  const primary = await latestTx(connection, winner, [claimInstruction], [winner]);
  const racing = await latestTx(connection, winner, [claimInstruction, SystemProgram.transfer({ fromPubkey: winner.publicKey, toPubkey: winner.publicKey, lamports: 1 })], [winner]);
  ...
  const a = await reconcile(connection, primary.signature);
  const b = await reconcile(connection, racing.signature);
  const landed = [primary.signature, racing.signature].filter((_sig, index) => [a, b][index].state === 'landed');
  ...
  return {
    ...
    racingSignature: racing.signature,
    primaryReconciliation: a,
    racingReconciliation: b,
    successfulSignature: landed[0],
    exactlyOneLanded: true,
    lostResponseRecoveredBySignatureReconciliation: true,
  };
}
...
function entryIx(entrant, competitionId, pool, asset, receipt) {
  return ix('deposit_competition_entry_v2', [
    { pubkey: entrant.publicKey, isSigner: true, isWritable: true },
    { pubkey: CONFIG, isSigner: false, isWritable: false },
    { pubkey: pool, isSigner: false, isWritable: true },
...
function resolveIx(resolver, competitionId, pool, winnerAsset, winnerWallet, winnerReceipt = SYS) {
  return ix('resolve_competition_pool_v2', [
    ...
    { pubkey: CONFIG, isSigner: false, isWritable: false },
    { pubkey: pool, isSigner: false, isWritable: true },
    { pubkey: winnerReceipt, isSigner: false, isWritable: false },
  ], [competitionId, pb(winnerAsset), pb(winnerWallet)]);
}
function claimIx(winner, competitionId, pool) {
  return ix('claim_competition_winner_v2', [
...
async function main() {
  ...
  assert(Number.isFinite(solUsd) && solUsd > 0, 'SOLANA_CLOSEOUT_SOL_USD must be positive');
  ...
  const duplicateBroadcast = await samePacketDuplicateProbe(connection, authority);
  const ambiguousResponse = await ambiguousResponseProbe(connection, authority);

  const staleResolver = Keypair.generate();
  const staleResolverRent = await connection.getMinimumBalanceForRentExemption(0, 'confirmed');
  const staleResolverFundingLamports = staleResolverRent + 5_000_000;
  const participantFunding = [ownerA, ownerB].map((wallet) => SystemProgram.transfer({
    fromPubkey: authority.publicKey,
    toPubkey: wallet.publicKey,
    lamports: Number(battleEntry + 5_000_000n),
  }));
  participantFunding.push(SystemProgram.transfer({ fromPubkey: authority.publicKey, toPubkey: staleResolver.publicKey, lamports: staleResolverFundingLamports }));
  const fundingSig = await send(connection, authority, participantFunding, [authority]);
  assert(await connection.getBalance(staleResolver.publicKey, 'confirmed') >= staleResolverRent, 'stale resolver was not rent-exempt for an authorization-level rejection');

  const battleId = id('solana-postbond-battle');
  ...
  const battleLive = readPool(await account(connection, battlePool));
  assert(battleLive.state === 1 && battleLive.entryCount === 2, 'Battle did not become LIVE after two entries');
  ...
  assert(plannedWinner === 'left' || plannedWinner === 'right', 'Battle winner must be left or right');
  const battleWinner = plannedWinner === 'right' ? ownerB : ownerA;
  const battleWinnerMint = plannedWinner === 'right' ? mintB : mintA;
  const battleLoser = plannedWinner === 'right' ? ownerA : ownerB;
  ...
  const tournamentA = Keypair.generate();
  const tournamentB = Keypair.generate();
  const tournamentFunding = await send(connection, authority, [tournamentA, tournamentB].map((wallet) => SystemProgram.transfer({
    fromPubkey: authority.publicKey,
    toPubkey: wallet.publicKey,
    lamports: Number(tournamentEntry + 5_000_000n),
  })), [authority]);
  const tournamentId = id('solana-normal-tournament');
  ...
  const tournamentReceiptA = pda([ENTRY_SEED, tournamentId, mintA.toBuffer(), tournamentA.publicKey.toBuffer()]);
  const tournamentReceiptB = pda([ENTRY_SEED, tournamentId, mintB.toBuffer(), tournamentB.publicKey.toBuffer()]);
  const tournamentEntryA = await send(connection, tournamentA, [entryIx(tournamentA, tournamentId, tournamentPool, mintA, tournamentReceiptA)], [tournamentA]);
  const tournamentEntryB = await send(connection, tournamentB, [entryIx(tournamentB, tournamentId, tournamentPool, mintB, tournamentReceiptB)], [tournamentB]);
  const tournamentResolve = await send(connection, resolver, [resolveIx(resolver, tournamentId, tournamentPool, mintA, tournamentA.publicKey, tournamentReceiptA)], [resolver]);
  ...
  const voteFunding = await send(connection, authority, [SystemProgram.transfer({
    fromPubkey: authority.publicKey,
    toPubkey: vote.publicKey,
    lamports: Number(voteEntry + oneDollarBoost + 8_000_000n),
  })], [authority]);
  const voteId = id('solana-vote-tournament');
  const votePool = pda([POOL_SEED, voteId]);
  ...
  const boostFundingId = id('vote-one-dollar-boost');
  ...
  const reload = {
    battle: readPool(await account(reloadConnection, battlePool)),
    tournament: readPool(await account(reloadConnection, tournamentPool)),
    voteTournament: readPool(await account(reloadConnection, votePool)),
  };
  ...
  const result = {
    schemaVersion: 2,
    purpose: 'solana-postbond-current-head-arena-money',
    ...
    config: cfg,
    economics: {
      competition: '75/20/5', voteBoost: '90/10/0', battleEntryLamports: String(battleEntry),
      normalTournamentEntryLamports: String(tournamentEntry), voteTournamentEntryLamports: String(voteEntry),
      oneDollarBoostLamports: String(oneDollarBoost), solUsd,
    },
    funding: { creatorsAndStaleResolver: fundingSig, tournamentEntrants: tournamentFunding, voteEntrant: voteFunding },
    battle: {
      id: battleId.toString('hex'), pool: battlePool.toBase58(), mintA: mintA.toBase58(), mintB: mintB.toBase58(),
      ownerA: ownerA.publicKey.toBase58(), ownerB: ownerB.publicKey.toBase58(), open: battleOpen, entryA, entryB,
      duplicateEntryRejected, staleResolver: staleResolver.publicKey.toBase58(), staleAuthorizationRejected,
      winnerSide: plannedWinner, resolve: battleResolve, wrongRecipientRejected, claim: battleClaim,
      duplicateClaimRejected, finalState: reload.battle,
    },
    normalTournament: {
      id: tournamentId.toString('hex'), pool: tournamentPool.toBase58(), open: tournamentOpen,
      registration: [tournamentEntryA, tournamentEntryB], winnerAsset: mintA.toBase58(), winnerWallet: tournamentA.publicKey.toBase58(),
      resolve: tournamentResolve, wrongRecipientRejected: tournamentWrongRecipient, claim: tournamentClaim,
      replayRejected: tournamentReplayRejected, finalState: reload.tournament,
    },
    voteTournament: {
      id: voteId.toString('hex'), pool: votePool.toBase58(), open: voteOpen, entry: voteEntrySig,
      boost: voteBoostSig, boostReceipt: boostReceipt.toBase58(), boostReplayRejected,
      boostSplit: { gross: voteState.boostGross, prize: voteState.boostPrize, protocol: voteState.boostProtocol },
      winnerAsset: mintB.toBase58(), winnerWallet: vote.publicKey.toBase58(), resolve: voteResolve,
      claim: voteClaim, claimReplayRejected: voteClaimReplayRejected, finalState: reload.voteTournament,
    },
    recovery: {
      duplicateBroadcast,
      ambiguousResponse,
      ...
      wrongChainRejectedByCanonicalChainGate: true,
      freshProcessReload: true,
      noDuplicateFinancialEffect: true,
    },
  };
