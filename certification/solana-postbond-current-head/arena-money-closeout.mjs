...
async function main() {
  ...
  participantFunding.push(SystemProgram.transfer({ fromPubkey: authority.publicKey, toPubkey: staleResolver.publicKey, lamports: 50_000 }));
  const fundingSig = await send(connection, authority, participantFunding, [authority]);
  assert(await connection.getBalance(staleResolver.publicKey, 'confirmed') >= 10_000, 'stale resolver was not funded for an authorization-level rejection');
